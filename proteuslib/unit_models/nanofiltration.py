...
# REVIEW: Imported Pyomo intrinsic "log" function here [yes]
from pyomo.environ import Var, Constraint, Param, Expression, SolverFactory, \
    TerminationCondition, Suffix, NonNegativeReals, units as pyunits, Reference, log
...
@declare_process_block_class("NF")
...
#        super(NFData, self).build()
        ...
        # REVIEW: REFLECTION COEFFICIENT SIGMA
        # COMMENT: Parameter value taken from Nair et al. (2018) for ESNA Membranes for Na+/Cl- ions
        # COMMENT: Technically a parameter but implemented as variable for simplified user manipulation (fixing/unfixing) [yes]
        self.sigma = Var(
            initialize=0.28,
        ...
        self.area = Var(
            ...
            doc='Membrane area')
        # REVIEW: AVERAGE CONCENTRATION FEED INLET
        # COMMENT: Adopted from Bruggen (2013)
        # QUESTION: Is the conc_mass_comp also in unit [kg/m**3]? Did I express the unit here properly? [yes]
        self.avg_concentration_in = Var(
            self.flowsheet().config.time,
            initialize=1e-3,
            bounds=(1e-8, 1e6),
            domain=NonNegativeReals,
            units=units_meta('mass')*units_meta('length')**-3,
            doc='Average concentration at feed inlet')
        self.avg_concentration_out = Var(
            self.flowsheet().config.time,
            initialize=1e-3,
            bounds=(1e-8, 1e6),
            domain=NonNegativeReals,
            units=units_meta('mass')*units_meta('length')**-3,
            doc='Average concentration at feed outlet')
        ...
        # mass transfer
        # redundant, but useful to have on NF block, reference doesn't work with units at the moment
        ...
        # REVIEW: MODIFIED FLUX PERFORMANCE EQUATIONS (WATER: SIGMA, SALT: SIGMA, FLUX, AVG_CONCENTRATION)
        # COMMENT: The average concentration avg_concentration_in/avg_concentration_out is NOT defined over mulitple components j at this time
        @self.Constraint(self.flowsheet().config.time,
                         self.config.property_package.component_list,
            ...
            if j == 'H2O':
                return (b.flux_mass_comp_in[t, j] == b.A[t] * b.dens_H2O
                        * ((prop_feed.pressure - prop_perm.pressure)
                           - b.sigma * (prop_feed.pressure_osm - prop_perm.pressure_osm)))
            elif j == 'NaCl':
                return (b.flux_mass_comp_in[t, j] == b.B[t]
                        * (prop_feed.conc_mass_comp[j] - prop_perm.conc_mass_comp[j]) + (1-b.sigma) * b.flux_mass_comp_in[t,j] * 1/b.dens_H2O * b.avg_concentration_in[t])
        ...
        def eq_flux_out(b, t, j):
            ...
            if j == 'H2O':
                ...
                return (b.flux_mass_comp_out[t, j] == b.B[t]
                        * (prop_feed.conc_mass_comp[j] - prop_perm.conc_mass_comp[j]) + (1-b.sigma) * b.flux_mass_comp_out[t,j] * 1/b.dens_H2O * b.avg_concentration_out[t])
        
        # Average concentration 
        # REVIEW: Average concentration declaration/expression/constraint
        # COMMENT: Chen approximation of logarithmic average implemented
        # QUESTION: Can I just import Pyomo intrinsic log function (see above) or should I use the numpy package along with operator "np.log" for "ln"? [yes-Pyomo]
        # QUESTION: Is it advantageous to define the average concentration as an expression? [depends]
        # QUESTION: Do I need to index this constraint over time? [yes]
        @self.Constraint(self.flowsheet().config.time,
                         self.config.property_package.component_list,
                         doc="Average inlet concentration")
        def eq_avg_conc_in(b,t,j):
            prop_feed = b.feed_side.properties_in[t]
            prop_perm = b.properties_permeate[t]
            if j == 'H2O':
                return Constraint.Skip
            elif j == 'NaCl':
                return (b.avg_concentration_in[t] == (prop_feed.conc_mass_comp[j] * prop_perm.conc_mass_comp[j] * (prop_feed.conc_mass_comp[j] + prop_perm.conc_mass_comp[j])/2)**(1/3))
        
        @self.Constraint(self.flowsheet().config.time,
                         self.config.property_package.component_list,
                         doc="Average outlet concentration")
        def eq_avg_conc_out(b,t,j):
            prop_feed = b.feed_side.properties_out[t]
            prop_perm = b.properties_permeate[t]
            if j == 'H2O':
                return Constraint.Skip
            elif j == 'NaCl':
                return (b.avg_concentration_out[t] == (prop_feed.conc_mass_comp[j] * prop_perm.conc_mass_comp[j] * (prop_feed.conc_mass_comp[j] + prop_perm.conc_mass_comp[j])/2)**(1/3))        
    ...
    def initialize(
        ...
        Keyword Arguments:
            routine : str stating which initialization routine to execute
                        * None - currently no specialized routine for NF unit
