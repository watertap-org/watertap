...
class Test_initialization_args():
    ...
    def m(self):
        ...
        m.fs.unit.N_Re_io[0, 'in'].fix(400)
    ...
    def test_initialize_ignore_dof(self, m):
        ...
        temp = m.fs.unit.N_Re_io[0, 'in'].value
        m.fs.unit.N_Re_io[0, 'in'].unfix()
    ...
    def test_initialize_fail_on_warning(self, m):
        ...
        temp = m.fs.unit.inlet.pressure[0].value
        m.fs.unit.inlet.pressure[0].fix(0.1 * temp)
        # Next line should pass since only a warning is given
        m.fs.unit.initialize(ignore_dof=True, fail_on_warning=False)
        # Set fail_on_warning to True
        with pytest.raises(ValueError, match="Initialization Step 3 failed. The solver failed to "
