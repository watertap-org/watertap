...
# This config is REQUIRED to use EquilibriumReactor even if
# we have no equilibrium reactions
_reaction_config = {
    "base_units": {
        "time": pyunits.s,
        "length": pyunits.m,
...
class TestReactions:
    """Base class for tests, that sets config files.
    """
    thermo_config = _water_thermo_config
    reaction_config = _reaction_config


# Start test class
class TestENRTLwater(TestReactions):
    @pytest.fixture(scope="class")
    def water_model(self):
        model = ConcreteModel()
        model.fs = FlowsheetBlock(default={"dynamic": False})
        model.fs.thermo_params = GenericParameterBlock(default=self.thermo_config)
        model.fs.rxn_params = GenericReactionParameterBlock(
            default={"property_package": model.fs.thermo_params, **self.reaction_config}
        )
    ...
    def test_solution_water(self, water_model):
        ...
        pH = -log10(
            value(
                model.fs.unit.control_volume.properties_out[0.0].act_phase_comp[
                    "Liq", "H_+"
                ]
            )
            * 55.2
...
class TestENRTLcarbonicAcid(TestReactions):
    thermo_config = carbonic_thermo_config
    ...
    def carbonic_acid_model(self):
        ...
        model.fs.unit.inlet.mole_frac_comp[0, "H2O"].fix(
            1.0 - 4 * zero - acid - value(model.fs.unit.inlet.mole_frac_comp[0, "Na_+"])
        )
        model.fs.unit.inlet.pressure.fix(101325.0)
        model.fs.unit.inlet.temperature.fix(298.0)
        model.fs.unit.inlet.flow_mol.fix(10)
    ...
    def test_solution_carbonic_acid(self, carbonic_acid_model):
        ...
        gamma = {}
        for index in model.fs.unit.control_volume.properties_out[
            0.0
        ].conc_mol_phase_comp:
            gamma[index] = value(
                model.fs.unit.control_volume.properties_out[0.0].act_phase_comp[index]
            ) / value(model.fs.unit.outlet.mole_frac_comp[0, index[1]])
        ...
        tolerance = 1e-4  # was: 1e-5, but this (barely) failed on Windows
        assert pytest.approx(0.002061178349769601, rel=tolerance) == total_acid
