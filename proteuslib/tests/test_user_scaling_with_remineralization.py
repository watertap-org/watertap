...
class TestRemineralizationCSTR():
    ...
    def test_scaling_cstr_kin(self, remineralization_cstr_kin):
        ...
        for i in model.fs.rxn_params.rate_reaction_idx:
            ...
            iscale.set_scaling_factor(model.fs.unit.control_volume.rate_reaction_extent[0.0,i], 10/(scale+1e-6))
        ...
        # NOTE: Give an initial guess of pH 7 and inlet values or 1e-6 for others
        ...
        for c in iscale.unscaled_constraints_generator(model):
            print(f"    {c}")
        print("Scaled constraints by factor:")
        for c, s in iscale.constraints_with_scale_factor_generator(model):
            print(f"    {c}, {s}")
        print("Badly scaled variables:")
        for v, sv in iscale.badly_scaled_var_generator(model, large=1e2, small=1e-2, zero=1e-12):
