...
sweep_params['System Recovery'] = LinearSample(m.fs.system_recovery_target, 0.5, 0.95, 30)
#sweep_params['Max RO Pressure'] = LinearSample(m.fs.max_allowable_pressure, 65e5, 300e5, 20)
sweep_params['Max RO Pressure'] = LinearSample(m.fs.max_allowable_pressure, 300e5, 65e5, 30)
...
global_results = parameter_sweep(m, sweep_params, outputs, output_filename, optimize_function=optimize, debugging_data_dir='output', reinitialize_function=solve_optimization, reinitialize_kwargs=kwargs_flowsheet)
...
if rank == 0:
    ...
    for k in range(len(outputs)):
        ...
        yi = griddata(x0, y0, xi, method='linear')
        global_results_clean[:, nc + k] = yi

    np.savetxt('%s_clean.csv' % (output_filename.split('.')[0]), global_results_clean, delimiter=',')
