...
import examples.flowsheets.full_treatment_train.model_components.seawater_salt_prop_pack as property_seawater_salts
...
def test_property_seawater_ions():
    ...
    for s in feed_mass_frac:
        m.fs.stream[0].flow_mass_phase_comp["Liq", s].fix(
            feed_flow_mass * feed_mass_frac[s]
        )
    ...
    assert value(m.fs.stream[0].flow_mol_phase_comp["Liq", "Na"]) == pytest.approx(
        0.4838, rel=1e-3
    )
    assert value(m.fs.stream[0].flow_mol_phase_comp["Liq", "Ca"]) == pytest.approx(
        9.531e-3, rel=1e-3
    )
...
class TestPropertySeawaterIons(PropertyTestHarness):
    def configure(self):
        ...
        }  # 8 state vars, pressure and temp are not active
        self.default_solution = {
            ("mass_frac_phase_comp", ("Liq", "H2O")): 0.9647,
            ("flow_vol", None): 1e-3,
...
class TestPropertySeawaterSalts(PropertyTestHarness):
    def configure(self):
        ...
        }  # 7 state vars, pressure and temp are not active
        self.default_solution = {
            ("mass_frac_phase_comp", ("Liq", "H2O")): 0.9647,
            ("flow_vol", None): 1e-3,
