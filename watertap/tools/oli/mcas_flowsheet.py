...
def mcas_flowsheet(source_water: dict):
    ...
    for key in ["components", "temperature", "pressure"]:
        if key not in source_water.keys():
            raise RuntimeError(
                f" Key '{key}' missing from source_water. Initialization aborted."
            )

    components = {
        watertap_to_oli(comp): source_water["components"][comp]
...
def create_mcas_input(components):
    ...
    mcas_input = {
        "solute_list": [comp.oli_name for comp in components],
        "mw_data": {comp.oli_name: comp.molar_mass * 1e-3 for comp in components},
        "charge": {
            comp.oli_name: comp.charge for comp in components if comp.charge != 0
        },
    }
...
def calculate_mass_frac_phase_comp(
    components: dict, volume: float = 1, density: float = 1e6
):
    """
