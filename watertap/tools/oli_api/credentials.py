...
class CredentialManager:
    ...
    def __init__(
        ...
        self._manage_credentials(username, password, root_url, auth_url, access_keys)
        if not self.test:
            self.login()
    ...
    def _set_config_file(self, config_file):
        ...
        if not self.test:
            ...
            self.config_file = join(Path(__file__).parents[0], "tests", "test_credentials.txt")
    ...
    def _manage_credentials(self, username, password, root_url, auth_url, access_keys):
        ...
        if self.encryption_key:
            ...
            if not access_keys:
                self._check_credentials(["username", "password", "root_url", "auth_url"])
            else:
    ...
    def generate_oliapi_access_key(self):
        ...
        self.credentials["access_keys"].append(json.loads(response.text)["data"]["apiKey"])
        if not self.test:
            print(response.text)
            ...
            if req_result.status_code == 200:
                ...
                if "access_token" in req_result:
                    ...
                    if "refresh_token" in req_result:
                        ...
                        return True
        if not self.test:
            raise ConnectionError(f" OLI login failed. Status code is {req_result.status_code}.")
        else:
            return False
    ...
    def request_auto_login(self, req_func=None):
        ...
        while num_tries <= 3:
            ...
            if req_result.status_code == 200:
                ...
                req_result = req_result.json()
                if not self.login():
                    raise RuntimeError("Login failed. Please check your API access key.\n")
