...
class WaterTAPCostingData(FlowsheetCostingBlockData):
    ...
    def build_process_costs(self):
        ...
        if (pyo.units.get_units(sum(self.aggregate_flow_costs.values()))) == pyo.units.dimensionless:
            self.total_operating_cost_constraint = pyo.Constraint(
                ...
                + sum(self.aggregate_flow_costs.values()) * self.base_currency / self.base_period * self.utilization_factor
            )
        else:
              self.total_operating_cost_constraint = pyo.Constraint(
                expr=self.total_operating_cost
