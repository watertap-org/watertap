...
class TestElectroNPFlowsheet:
    ...
    def test_results(self, model):
        ...
        ) == pytest.approx(12.292, rel=1e-4)
        assert value(
            model.fs.electroNP.treated.conc_mass_comp[0, "S_F"]
        ) == pytest.approx(31.734, rel=1e-4)
        assert value(
        ...
        ) == pytest.approx(0.38462, rel=1e-4)
        assert value(
        ...
        ) == pytest.approx(0.23651, rel=1e-4)
