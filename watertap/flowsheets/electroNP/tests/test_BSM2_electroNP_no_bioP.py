...
class TestElectroNPFlowsheet:
    ...
    def test_results(self, model):
        ...
        ) == pytest.approx(8.712, rel=1e-4)
        assert value(
            model.fs.electroNP.treated.conc_mass_comp[0, "S_F"]
        ) == pytest.approx(22.759, rel=1e-4)
        assert value(
        ...
        ) == pytest.approx(1.46712, rel=1e-4)
        assert value(
        ...
        ) == pytest.approx(0.321073, rel=1e-4)
        assert value(
        ...
        ) == pytest.approx(0.065616, rel=1e-4)
