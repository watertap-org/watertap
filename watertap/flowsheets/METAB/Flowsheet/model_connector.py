...
def get_mw_data():
    # Read the CSV file and set 'Symbol' as the index
    df = pd.read_csv("mw.csv", index_col="Symbol")
...
def get_metab_data():
    ...
    df = pd.read_csv("metab_input.csv", index_col=0).dropna(axis=1)
...
def get_mw(df, symbol):
    try:
        weight = df.loc[symbol, "Molecular Weight (g/mol)"]
        return float(weight)
    except KeyError:
        return f"Symbol '{symbol}' not found."


def outlet_unit_con(outlet, mw, total_vol):
    value = outlet * 1000 * 24 * mw / total_vol
    return float(value)  # g/m^3


def get_influent(m):

    df = pd.read_csv("influent_to_wwtp.csv", index_col=0).dropna(axis=1)
...
def build_metab(m, mass=False, idx=1):
    ...
    if mass:
        ...
        m.fs.metab_effluent.inlet.flow_vol.fix(
            float(get_outlet(df_metab, idx, "VolumetricFlowrate"))
            * units.m**3
            / units.day
        )
        ...
        for i in comp_list:
            if i == "S_cat":
                m.fs.metab_effluent.inlet.cations[0].fix(
                    float(get_outlet(df_metab, idx, i)) * units.mmol / units.liter
                )
                ...
                m.fs.metab_effluent.inlet.conc_mass_comp[0, i].fix(
                    get_outlet(df_metab, idx, i) * units.mg / units.liter
                )
    else:
        df_mw = get_mw_data()
        df_metab = get_metab_data()
        m.fs.metab_effluent = Translator_ADM1_ASM1(
            inlet_property_package=m.fs.props_adm1,
            outlet_property_package=m.fs.props_asm1,
            reaction_package=m.fs.adm1_rxn_props,
        ...
        print(df_metab)
        print(get_outlet(df_metab, idx, comp))
        ...
        for i in comp_list:
            total_mass = (
                total_mass + get_outlet(df_metab, idx, i) * get_mw(df_mw, i) * 24
            )  # kg/day
        total_vol = float(total_mass) / 997  # kg/m3 100 to scale up to connect
        ...
        m.fs.metab_effluent.inlet.flow_vol.fix(
            float(total_vol) * units.m**3 / units.day
        )
        m.fs.metab_effluent.inlet.temperature.fix(308.15 * units.K)
        m.fs.metab_effluent.inlet.pressure.fix(1 * units.atm)
        ...
        for i in comp_list:
            if i == "S_cat":
                m.fs.metab_effluent.inlet.cations[0].fix(
                    float(get_outlet(df_metab, idx, i) * 1000 * 24 / total_vol)
                    * units.mol
                    / units.m**3
                )
                ...
                m.fs.metab_effluent.inlet.conc_mass_comp[0, i].fix(
                    outlet_unit_con(
                        get_outlet(df_metab, idx, i), get_mw(df_mw, i), total_vol
                    )
                    * units.g
                    / units.m**3
                )
