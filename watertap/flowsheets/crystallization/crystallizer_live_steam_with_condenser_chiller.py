...
def build():
    ...
    m.fs.s02 = Arc(source=m.fs.separator.recycle, destination=m.fs.mixer.recycle)
    ...
    m.fs.s10 = Arc(
        source=m.fs.tb_vapor.outlet, destination=m.fs.condenser.hot_side_inlet
    )
    m.fs.s11 = Arc(
        source=m.fs.condenser.hot_side_outlet, destination=m.fs.distillate.inlet
    )
    m.fs.s12 = Arc(
...
def initialize_system(m, solver=None, verbose=True):
    ...
    m.fs.condenser.hot_side_inlet.temperature[0] = m.fs.crystallizer.vapor.temperature[
        0
    ].value
    m.fs.condenser.hot_side_inlet.pressure[0] = m.fs.crystallizer.vapor.pressure[
        0
    ].value
