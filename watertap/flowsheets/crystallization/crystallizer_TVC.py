...
def build():
    ...
    m.fs.s13 = Arc(
        source=m.fs.separator.recycle, destination=m.fs.mixer.recycle)
    ...
    m.fs.s08 = Arc(source=m.fs.tb_vapor.outlet, destination=m.fs.separator_vapor.inlet) ###change in arcs for TVC starts here
    m.fs.s09 = Arc(source=m.fs.separator_vapor.secondary_steam, destination=m.fs.SteamEjector.inlet_entrained_vapor)
    m.fs.s10 = Arc(source=m.fs.SteamEjector.outlet_discharge_mix, destination=m.fs.heater.hot_side_inlet)
    m.fs.s12 = Arc(source=m.fs.heater.hot_side_outlet, destination=m.fs.distillate_heater.inlet)
    m.fs.s14 = Arc(source=m.fs.separator_vapor.residual_vapor, destination=m.fs.condenser.hot_side_inlet) #new arcs for TVC added here and onward
    m.fs.s15 = Arc(source=m.fs.condenser.hot_side_outlet, destination=m.fs.distillate_condenser.inlet)
    m.fs.s16 = Arc(source=m.fs.condenser.cold_side_outlet, destination=m.fs.chiller.inlet)
...
def add_costs(m):
   ...
   # )
    ...
    m.fs.costing.add_annual_water_production(m.fs.distillate_heater.properties[0].flow_vol + m.fs.distillate_condenser.properties[0].flow_vol)
    m.fs.costing.add_specific_energy_consumption(m.fs.distillate_heater.properties[0].flow_vol + m.fs.distillate_condenser.properties[0].flow_vol)
...
def set_operating_conditions(m):
    ...
    m.fs.SteamEjector.properties_motive_steam[0].flow_mass_phase_comp["Liq", "H2O"].fix(0)
    ...
    m.fs.SteamEjector.properties_entrained_vapor[0].flow_mass_phase_comp["Liq", "H2O"].fix(0)
...
def initialize_system(m, solver=None, verbose=True):
    ...
    m.fs.separator_vapor.inlet.flow_mass_phase_comp[0, "Liq", "H2O"] = m.fs.crystallizer.vapor.flow_mass_phase_comp[0, "Liq", "H2O"].value
    m.fs.separator_vapor.inlet.flow_mass_phase_comp[0, "Vap", "H2O"] = m.fs.crystallizer.vapor.flow_mass_phase_comp[0, "Vap", "H2O"].value
    m.fs.separator_vapor.inlet.temperature[0] = m.fs.crystallizer.vapor.temperature[0].value
    ...
    m.fs.SteamEjector.properties_entrained_vapor[0].flow_mass_phase_comp["Vap", "H2O"].fix(m.fs.separator_vapor.secondary_steam.flow_mass_phase_comp[0, "Vap", "H2O"].value)
    ...
    m.fs.SteamEjector.properties_entrained_vapor[0].flow_mass_phase_comp["Vap", "H2O"].unfix()
    ...
    # Separator
    m.fs.separator.inlet.flow_mass_phase_comp[0, "Liq", "H2O"] = m.fs.crystallizer.outlet.flow_mass_phase_comp[0, "Liq", "H2O"].value
    m.fs.separator.inlet.flow_mass_phase_comp[0, "Liq", "NaCl"] = m.fs.crystallizer.outlet.flow_mass_phase_comp[0, "Liq", "NaCl"].value
    ...
    m.fs.condenser.hot_side_inlet.flow_mass_phase_comp[0, "Liq", "H2O"] = m.fs.separator_vapor.residual_vapor.flow_mass_phase_comp[0, "Liq", "H2O"].value
    m.fs.condenser.hot_side_inlet.flow_mass_phase_comp[0, "Vap", "H2O"] = m.fs.separator_vapor.residual_vapor.flow_mass_phase_comp[0, "Vap", "H2O"].value
    m.fs.condenser.hot_side_inlet.temperature[0] = m.fs.separator_vapor.residual_vapor.temperature[0].value
    m.fs.condenser.hot_side_inlet.pressure[0] = m.fs.separator_vapor.residual_vapor.pressure[0].value
...
def display_system(m):
    ...
    print("outlet temperature condenser:", m.fs.condenser.cold_side_outlet.temperature[0].value)
    print("outlet temperature condenser:", m.fs.condenser.cold_side_inlet.flow_mass_phase_comp[0, "Liq", "H2O"].value)
    print("thermocompressor pressure ratio:", m.fs.SteamEjector.compression_ratio.value)
    print("separator split recycle:", m.fs.separator.recycle.flow_mass_phase_comp[0,"Liq", "H2O"].value)
    print("separator split purge:", m.fs.separator.purge.flow_mass_phase_comp[0,"Liq", "H2O"].value)
