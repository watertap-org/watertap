...
def add_costs(m):
    ...
    m.fs.condenser.costing = UnitModelCostingBlock(
        flowsheet_costing_block=m.fs.costing, costing_method=cost_heat_exchanger
    )
...
def initialize_system(m, solver=None, verbose=True):
    ...
    m.fs.condenser.hot_side_inlet.flow_mass_phase_comp[0, "Liq", "H2O"] = (
        m.fs.separator_vapor.secondary_steam.flow_mass_phase_comp[0, "Liq", "H2O"].value
    )
    ...
    m.fs.condenser.hot_side_inlet.temperature[0] = (
        m.fs.separator_vapor.secondary_steam.temperature[0].value
    )
    m.fs.condenser.hot_side_inlet.pressure[0] = (
        m.fs.separator_vapor.secondary_steam.pressure[0].value
    )
