...
def set_operating_conditions(m, cc_configuration=None, **kwargs):
    ...
    if m.fs.operation_mode == "filtration":
        ...
        if cc_configuration["flushing_flowrate"] == "raw_feed_flowrate":
            m.fs.conduit_feed.properties[0].flow_vol_phase["Liq"].fix(
                cc_configuration["raw_feed_flowrate"]
            )
        elif cc_configuration["flushing_flowrate"] == "recycle_flowrate":
            m.fs.conduit_feed.properties[0].flow_vol_phase["Liq"].fix(
        ...
        if cc_configuration["flushing_conc"] == "raw_feed_conc":
            m.fs.conduit_feed.properties[0].conc_mass_phase_comp["Liq", "NaCl"].fix(
                cc_configuration["raw_feed_conc"]
            )
        else:
            m.fs.conduit_feed.properties[0].conc_mass_phase_comp["Liq", "NaCl"].fix(
        ...
        m.fs.conduit_feed.properties[0].temperature.fix(cc_configuration["temperature"])
        m.fs.conduit_feed.properties[0].pressure.fix(101325 * pyunits.Pa)
        m.fs.conduit_feed.properties[0].flow_vol_phase["Liq"]
        m.fs.conduit_feed.properties[0].conc_mass_phase_comp["Liq", "NaCl"]
        solver = get_solver()
        solver.solve(m.fs.conduit_feed)
        # Dead Volume operating conditions
        # Fix volume
        if cc_configuration["dead_volume"] == "base_on_dead_volume_to_area_ratio":
            dead_volume = (
