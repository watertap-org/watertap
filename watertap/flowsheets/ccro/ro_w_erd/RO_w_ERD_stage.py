...
def build_model(blk, m=None,
    ...
    blk.P1 = Pump(property_package=m.fs.properties)
    ...
    blk.ERD = EnergyRecoveryDevice(property_package=m.fs.properties)

    blk.product = StateJunction(property_package=m.fs.properties)
    blk.disposal = StateJunction(property_package=m.fs.properties)

    # Connections
    blk.feed_to_P1 = Arc(source=blk.feed.outlet, destination=blk.P1.inlet)
    blk.P1_to_RO = Arc(source=blk.P1.outlet, destination=blk.RO.inlet)
    blk.RO_to_ERD = Arc(source=blk.RO.retentate, destination=blk.ERD.inlet)
    blk.ERD_to_disposal = Arc(source=blk.ERD.outlet, destination=blk.disposal.inlet)
