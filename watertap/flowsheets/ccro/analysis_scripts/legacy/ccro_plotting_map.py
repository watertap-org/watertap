...
if __name__ == "__main__":
    data_manager = psDataManager(
        [
            "output/ccro_map_sweep_analysisType_study_BGW_map.h5",
        ]
    )
    import_keys = [
        {
    ...
    for lp in line_plots_options:
        mp = MapPlotter(
            data_manager, save_folder="map_figs", save_name=f"BGW lcow map {lp}"
        )
        mp.plot_map(
            [("use_hold_up", lp)],
            "Water recovery",
            "Recycle flow",
            "LCOW",
            zlevels=[0.5, 0.6, 0.7, 0.8, 0.9, 1.0, 1.1],
            axis_options={
                "xlabel": "Water recovery (%)",
                "ylabel": "Recycle flow rate (L/s)",
                "zlabel": "LCOW ($\$$/m$^3$)",
                "xticklabels": [50, 60, 70, 80, 90],
                "yticklabels": [1, 5, 10, 15, 20],
                "zticks": [0, 0.2, 0.4, 0.6, 0.8, 1],
            },
        )
        mp = MapPlotter(
            data_manager, save_folder="map_figs", save_name=f"BGW area map {lp}"
        )
        mp.plot_map(
            [("use_hold_up", lp)],
            "Water recovery",
        ...
        mp = MapPlotter(
            data_manager, save_folder="map_figs", save_name=f"BGW cycle time map {lp}"
        )
        mp.plot_map(
            [("use_hold_up", lp)],
            "Water recovery",
        ...
        mp = MapPlotter(
            ...
            save_name=f"BGW flushing efficiency map {lp}",
        )
        mp.plot_map(
            [("use_hold_up", lp)],
            "Water recovery",
