...
def config_operating_dict(op_dict):
    """
    Dictionary of operating conditions with units
    """
    
    op_dict_with_units = op_dict

    # Raw feed concentration and flowrate with units
    op_dict_with_units["raw_feed_conc"] = op_dict["raw_feed_conc"] * pyunits.g/ pyunits.L
    op_dict_with_units["raw_feed_flowrate"] = (op_dict["raw_feed_flowrate"] * pyunits.L / pyunits.min)
    op_dict_with_units["temperature"] = op_dict["temperature"] * pyunits.degK

    # Recycle concentration and flowrate with units
    op_dict_with_units["recycle_flowrate"] = (op_dict["recycle_flowrate"] * pyunits.L / pyunits.min)
    op_dict_with_units["recycle_conc_start"] = op_dict["recycle_conc_start"] * pyunits.g/ pyunits.L
  
    # Flushing concentration and flowrates with unit
    op_dict_with_units["flushing_flowrate"] = (op_dict_with_units["raw_feed_flowrate"] + op_dict_with_units["recycle_flowrate"])
    op_dict_with_units["flushing_conc"] = op_dict["flushing_conc"] * pyunits.g / pyunits.L
    op_dict_with_units["flushing_efficiency"] = op_dict["flushing_efficiency"] * pyunits.dimensionless

    # Pump efficiencies and start pressures with units
    op_dict_with_units["p1_eff"] = op_dict["p1_eff"] * pyunits.dimensionless
    op_dict_with_units["p2_eff"] = op_dict["p2_eff"] * pyunits.dimensionless
    op_dict_with_units["p1_pressure_start"] = op_dict["p1_pressure_start"] * pyunits.psi
    op_dict_with_units["p2_pressure_start"] = op_dict["p2_pressure_start"] * pyunits.psi
    
    # Membrane properties with units
    op_dict_with_units["A_comp"] = op_dict["A_comp"] * pyunits.m / (pyunits.s * pyunits.Pa)
    op_dict_with_units["B_comp"] = op_dict["B_comp"] * pyunits.m / pyunits.s
    op_dict_with_units["membrane_area"] = op_dict["membrane_area"] * pyunits.m**2
    op_dict_with_units["membrane_length"] = op_dict["membrane_length"] * pyunits.m
    op_dict_with_units["channel_height"] = op_dict["channel_height"] * pyunits.m
    op_dict_with_units["spacer_porosity"] = op_dict["spacer_porosity"] * pyunits.dimensionless
    
    # Dead volume and accumulation time with units
    op_dict_with_units["dead_volume"] = op_dict["dead_volume"] * pyunits.m**3
    op_dict_with_units["accumulation_time"] = op_dict["accumulation_time"] * pyunits.second

    return op_dict_with_units
...
def set_operating_conditions(m, op_dict=None, **kwargs):
    ...
    if m.fs.operation_mode == "filtration":
        ...
        m.fs.raw_feed.properties[0].flow_vol_phase["Liq"].fix(op_dict["raw_feed_flowrate"])
        m.fs.raw_feed.properties[0].conc_mass_phase_comp["Liq", "NaCl"].fix(op_dict["raw_feed_conc"])
        ...
        m.fs.flushing.flushing_feed_concentration.fix(op_dict["flushing_conc"])
