...
class ReverseOsmosisData(_ReverseOsmosisBaseData):
    ...
    def build(self):
        ...
        self.permeate_side = ControlVolume0DBlock(
            default={
                "dynamic": False,
                "has_holdup": False,
                "property_package": self.config.property_package,
        ...
        # pressure change
        if self.config.has_pressure_change is True and self.config.momentum_balance_type != "none":
        ...
        def eq_permeate_production(b, t, p, j):
            return (
                b.permeate_side.properties_mixed[t].get_material_flow_terms(p, j)
        ...
        def eq_flux_io(b, t, x, p, j):
            ...
            if comp.is_solvent():
                ...
                return b.flux_mass_io_phase_comp[t, x, p, j] == b.B_comp[t, j] * (
                    prop_feed_inter.conc_mass_phase_comp[p, j]
        ...
        def eq_connect_enthalpy_transfer(b, t):
            return (
                b.permeate_side.properties_mixed[t].get_enthalpy_flow_terms("Liq")
                == -b.feed_side.enthalpy_transfer[t]
            )

        @self.Constraint(self.flowsheet().config.time, doc="Isothermal assumption for permeate")
        def eq_permeate_isothermal(b, t):
            return (
                b.feed_side.properties_out[t].temperature
                == b.permeate_side.properties_mixed[t].temperature
            )

        # # Permeate-side stateblocks
        @self.permeate_side.Constraint(
            self.flowsheet().config.time, self.io_list, solute_set, doc="Permeate mass fraction"
        )
        def eq_mass_frac_permeate_io(b, t, x, j):
            ...
            return (
                prop_io.mass_frac_phase_comp["Liq", j]
                * sum(
                    self.flux_mass_io_phase_comp[t, x, "Liq", jj]
        ...
        @self.permeate_side.Constraint(
            self.flowsheet().config.time, self.io_list, doc="Permeate temperature"
        )
        ...
        @self.permeate_side.Constraint(
            self.flowsheet().config.time, self.io_list, doc="Permeate pressure"
        )
        ...
        @self.permeate_side.Constraint(
            self.flowsheet().config.time, self.io_list, doc="Permeate flowrate"
        )
        ...
        @self.feed_side.Constraint(
            self.flowsheet().config.time, self.io_list, solute_set, doc="Concentration polarization"
        )
        def eq_concentration_polarization_io(b, t, x, j):
            ...
            if self.config.concentration_polarization_type == ConcentrationPolarizationType.none:
                ...
                return prop_interface_io.conc_mass_phase_comp[
                    "Liq", j
                ] == prop_io.conc_mass_phase_comp["Liq", j] * exp(
                    jw / self.Kf_io[t, x, j]
                ) - js / jw * (
                    exp(jw / self.Kf_io[t, x, j]) - 1
                )
        ...
        if self.config.mass_transfer_coefficient == MassTransferCoefficient.calculated:
            ...
            def eq_Kf_io(b, t, x, j):
                ...
                return b.Kf_io[t, x, j] * b.dh == prop_io.diffus_phase["Liq"] * b.N_Sh_io[t, x]

            @self.Constraint(self.flowsheet().config.time, self.io_list, doc="Sherwood number")
            def eq_N_Sh_io(b, t, x):
                return b.N_Sh_io[t, x] == 0.46 * (b.N_Re_io[t, x] * b.N_Sc_io[t, x]) ** 0.36

            @self.Constraint(self.flowsheet().config.time, self.io_list, doc="Schmidt number")
            def eq_N_Sc_io(b, t, x):
                if x == "in":
                    ...
                    prop_io = b.feed_side.properties_out[t]
                return (
                    b.N_Sc_io[t, x] * prop_io.dens_mass_phase["Liq"] * prop_io.diffus_phase["Liq"]
        ...
        if (
            ...
            @self.Constraint(self.flowsheet().config.time, self.io_list, doc="Reynolds number")
            def eq_N_Re_io(b, t, x):
                if x == "in":
                    ...
                    prop_io = b.feed_side.properties_out[t]
                return (
                    b.N_Re_io[t, x] * b.area_cross * prop_io.visc_d_phase["Liq"]
                    == sum(
                        prop_io.flow_mass_phase_comp["Liq", j]
        ...
        if self.config.pressure_change_type == PressureChangeType.fixed_per_unit_length:
            ...
            @self.Constraint(
                self.flowsheet().config.time, self.io_list, doc="Crossflow velocity constraint"
            )
            ...
            @self.Constraint(
                self.flowsheet().config.time, self.io_list, doc="Darcy friction factor constraint"
            )
            ...
            def eq_dP_dx_io(b, t, x):
                ...
                return (
                    b.dP_dx_io[t, x] * b.dh
                    == -0.5
                    * b.friction_factor_darcy_io[t, x]
        ...
        @self.feed_side.Constraint(
            self.flowsheet().config.time, self.io_list, doc="Temperature at interface"
        )
        def eq_equal_temp_interface_io(b, t, x):
            if x == "in":
                ...
                prop_io = b.properties_out[t]
                prop_interface_io = b.properties_interface_out[t]
            return prop_interface_io.temperature == prop_io.temperature

        @self.feed_side.Constraint(
            self.flowsheet().config.time, self.io_list, doc="Pressure at interface"
        )
        def eq_equal_pressure_interface_io(b, t, x):
            if x == "in":
                ...
                prop_io = b.properties_out[t]
                prop_interface_io = b.properties_interface_out[t]
            return prop_interface_io.pressure == prop_io.pressure

        @self.feed_side.Constraint(
            self.flowsheet().config.time, self.io_list, doc="Volumetric flow at interface of inlet"
        )
        def eq_equal_flow_vol_interface_io(b, t, x):
            if x == "in":
                ...
                prop_io = b.properties_out[t]
                prop_interface_io = b.properties_interface_out[t]
            return prop_interface_io.flow_vol_phase["Liq"] == prop_io.flow_vol_phase["Liq"]
        ...
        def eq_rejection_phase_comp(b, t, j):
            return b.rejection_phase_comp[t, "Liq", j] == 1 - (
                b.permeate_side.properties_mixed[t].conc_mass_phase_comp["Liq", j]
    ...
    def initialize(
        ...
        flags = blk.feed_side.properties_in.initialize(
            outlvl=outlvl, optarg=optarg, solver=solver, state_args=state_args, hold_state=True
        )
        ...
        if "flow_mass_phase_comp" not in state_args.keys():
            raise ConfigurationError(
                "ReverseOsmosis0D initialization routine expects "
        ...
        for j in blk.config.property_package.solute_set:
            state_args_interface_in["flow_mass_phase_comp"][("Liq", j)] *= initialize_guess[
                "cp_modulus"
            ]
            state_args_interface_out["flow_mass_phase_comp"][("Liq", j)] *= initialize_guess[
                "cp_modulus"
            ]

        blk.feed_side.properties_out.initialize(
            outlvl=outlvl, optarg=optarg, solver=solver, state_args=state_args_retentate,
        )
        blk.feed_side.properties_interface_in.initialize(
            outlvl=outlvl, optarg=optarg, solver=solver, state_args=state_args_interface_in,
        )
        blk.feed_side.properties_interface_out.initialize(
            outlvl=outlvl, optarg=optarg, solver=solver, state_args=state_args_interface_out,
        )
        blk.permeate_side.properties_mixed.initialize(
            outlvl=outlvl, optarg=optarg, solver=solver, state_args=state_args_permeate,
        )
        ...
        check_solve(
            res, checkpoint="Initialization Step 3", logger=init_log, fail_flag=fail_on_warning
        )
        # ---------------------------------------------------------------------
        # Release Inlet state
    ...
    def _get_performance_contents(self, time_point=0):
        ...
        for j in self.config.property_package.solute_set:
            ...
            var_dict[cin_mem_name] = self.feed_side.properties_interface_in[
                time_point
            ].conc_mass_phase_comp["Liq", j]
            cout_mem_name = f"{j} Concentration @Outlet,Membrane-Interface "
            var_dict[cout_mem_name] = self.feed_side.properties_interface_out[
                time_point
            ].conc_mass_phase_comp["Liq", j]
            cin_bulk_name = f"{j} Concentration @Inlet,Bulk "
            var_dict[cin_bulk_name] = self.feed_side.properties_in[time_point].conc_mass_phase_comp[
                "Liq", j
            ]
            cout_bulk_name = f"{j} Concentration @Outlet,Bulk "
            var_dict[cout_bulk_name] = self.feed_side.properties_out[
                time_point
            ].conc_mass_phase_comp["Liq", j]
            cp_name = f"{j} Permeate Concentration "
            var_dict[cp_name] = self.permeate_side.properties_mixed[
                time_point
            ].conc_mass_phase_comp["Liq", j]
        if self.feed_side.properties_interface_out[time_point].is_property_constructed(
            "pressure_osm"
        ):
            var_dict[
        ...
        if self.feed_side.properties_out[time_point].is_property_constructed("pressure_osm"):
            var_dict["Osmotic Pressure @Outlet,Bulk"] = self.feed_side.properties_out[
                time_point
            ].pressure_osm
        if self.feed_side.properties_interface_in[time_point].is_property_constructed(
            "pressure_osm"
        ):
            var_dict[
        ...
        if self.feed_side.properties_in[time_point].is_property_constructed("pressure_osm"):
            var_dict["Osmotic Pressure @Inlet,Bulk"] = self.feed_side.properties_in[
                time_point
            ].pressure_osm
        if self.feed_side.properties_in[time_point].is_property_constructed("flow_vol_phase"):
            var_dict["Volumetric Flowrate @Inlet"] = self.feed_side.properties_in[
                time_point
            ].flow_vol_phase["Liq"]
        if self.feed_side.properties_out[time_point].is_property_constructed("flow_vol_phase"):
            var_dict["Volumetric Flowrate @Outlet"] = self.feed_side.properties_out[
                time_point
            ].flow_vol_phase["Liq"]
    ...
    def calculate_scaling_factors(self):
        ...
        for sb in (
            self.permeate_side.properties_in,
            self.permeate_side.properties_out,
        ...
        for (t, x, p, j), v in self.flux_mass_io_phase_comp.items():
            if iscale.get_scaling_factor(v) is None:
                ...
                if comp.is_solvent():  # scaling based on solvent flux equation
                    ...
                    sf = iscale.get_scaling_factor(self.B_comp[t, j]) * iscale.get_scaling_factor(
        ...
        for (t, p, j), v in self.mass_transfer_phase_comp.items():
            if iscale.get_scaling_factor(v) is None:
                ...
                if comp.is_solute:
                    sf *= (
                        1e2  # solute typically has mass transfer 2 orders magnitude less than flow
                    )
                iscale.set_scaling_factor(v, sf)
        ...
        for ind, c in self.eq_mass_transfer_term.items():
            ...
            iscale.constraint_scaling_transform(c, sf / 10.0)
        ...
        for (t, x, j), c in self.permeate_side.eq_mass_frac_permeate_io.items():
            ...
            iscale.constraint_scaling_transform(c, sf * 100.0)
