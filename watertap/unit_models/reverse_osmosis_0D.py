...
class ReverseOsmosisData(_ReverseOsmosisBaseData):
    ...
    def build(self):
        ...
        def flux_mass_phase_comp_avg(b, t, p, j):
            return 0.5 * sum(
                b.flux_mass_io_phase_comp[t, x, p, j] for x in self.io_list
            )
        ...
        def eq_flux_io(b, t, x, p, j):
            ...
            if comp.is_solvent():
                return b.flux_mass_io_phase_comp[t, x, p, j] == b.A_comp[
                    t, j
                ] * b.dens_solvent * (
        ...
        def eq_flow_vol_permeate_io(b, t, x):
            ...
            return (
                ...
                == b.properties_mixed[t].flow_vol_phase["Liq"]
            )
        ...
        def eq_concentration_polarization_io(b, t, x, j):
            ...
            if (
                ...
                jw = (
                    self.flux_mass_io_phase_comp[t, x, "Liq", "H2O"] / self.dens_solvent
                )
        ...
        if self.config.mass_transfer_coefficient == MassTransferCoefficient.calculated:
            ...
            def eq_Kf_io(b, t, x, j):
                ...
                return (
                    b.Kf_io[t, x, j] * b.dh
                    == prop_io.diffus_phase["Liq"] * b.N_Sh_io[t, x]
                )

            @self.Constraint(
                self.flowsheet().config.time, self.io_list, doc="Sherwood number"
            )
            ...
            @self.Constraint(
                self.flowsheet().config.time, self.io_list, doc="Schmidt number"
            )
        ...
        if (
            ...
            @self.Constraint(
                self.flowsheet().config.time, self.io_list, doc="Reynolds number"
            )
        ...
        if self.config.pressure_change_type == PressureChangeType.fixed_per_unit_length:
            ...
            def eq_velocity_io(b, t, x):
                if x == "in":
                    ...
                    prop_io = b.feed_side.properties_out[t]
                return (
                    b.velocity_io[t, x] * b.area_cross == prop_io.flow_vol_phase["Liq"]
                )
            ...
            def eq_friction_factor_darcy_io(b, t, x):
                return (b.friction_factor_darcy_io[t, x] - 0.42) * b.N_Re_io[
                    t, x
                ] == 189.3
        ...
        def eq_equal_flow_vol_interface_io(b, t, x):
            if x == "in":
                ...
                prop_interface_io = b.properties_interface_out[t]
            return (
                prop_interface_io.flow_vol_phase["Liq"] == prop_io.flow_vol_phase["Liq"]
            )
    ...
    def initialize(
        ...
        state_args = blk._get_state_args(
            source, blk.permeate_side.properties_mixed[0], initialize_guess, state_args
        )
    ...
    def _get_performance_contents(self, time_point=0):
        ...
        for j in self.config.property_package.solute_set:
            ...
            var_dict[cin_bulk_name] = self.feed_side.properties_in[
                time_point
            ].conc_mass_phase_comp["Liq", j]
