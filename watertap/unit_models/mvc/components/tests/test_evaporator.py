...
@pytest.mark.requires_idaes_solver
@pytest.mark.component
def test_evaporator_with_MCAS_mass_flow():
    m = ConcreteModel()
    m.fs = FlowsheetBlock(dynamic=False)
...
@pytest.mark.requires_idaes_solver
@pytest.mark.component
def test_evaporator_with_MCAS_mole_flow():
    m = ConcreteModel()
    m.fs = FlowsheetBlock(dynamic=False)
    m.fs.properties_feed = MCASParameterBlock(
        solute_list=["Na_+", "Cl_-"],
    ...
    m.fs.properties_vapor.set_default_scaling(
        "flow_mol_phase_comp", 1, index=("Vap", "H2O")
    )
    m.fs.properties_vapor.set_default_scaling(
        "flow_mol_phase_comp", 1, index=("Liq", "H2O")
    )
    ...
    # state variables
    # Feed inlet
    m.fs.evaporator.inlet_feed.flow_mol_phase_comp[0, "Liq", "H2O"].fix(10 / 18e-3)
    m.fs.evaporator.inlet_feed.flow_mol_phase_comp[0, "Liq", "Na_+"].fix(0.05 / 23e-3)
    m.fs.evaporator.inlet_feed.flow_mol_phase_comp[0, "Liq", "Cl_-"].fix(0.076 / 35e-3)
    m.fs.evaporator.properties_feed[0].assert_electroneutrality(
        defined_state=True, adjust_by_ion="Cl_-"
    )
    m.fs.evaporator.inlet_feed.temperature[0].fix(273.15 + 50.52)  # K
