...
class IonExchangeODData(UnitModelBlockData):
    ...
    **Valid values:** {
    ...
    def build(self):
        ...
        self.vel_min_A = Param(
            initialize=150,
            units=pyunits.dimensionless # Perry's
        )

        self.vel_min_B = Param(
            initialize=1.75,
            units=pyunits.dimensionless # Perry's
        )
        ...
        self.underdrain_h = Param(
            initialize=0.5,
            units=pyunits.m,
            doc='Underdrain height [m]' # Perry's
        )

        self.distributor_h = Param(
            initialize=1.5,
            units=pyunits.m,
            doc='Distributor height [m]' # Perry's
        )
        ...
        self.resin_eq_capacity = Var(
            ...
            doc='Resin equilibrium capacity [mol/L]'
        )
        ...
        def eq_t_contact(b):
            return (b.t_contact == (b.bed_porosity * b.bed_depth) / b.vel_bed)

        @self.Constraint(doc='Contact time')

        def eq_t_contact2(b):
            prop_in = b.properties_in[0]
            return (b.t_contact == (b.bed_porosity * b.bed_vol) / prop_in.flow_vol_phase['Liq'])
        ...
        def eq_Re(b):
            prop_in = b.properties_in[0]
            return (b.Re == (b.bed_porosity * b.vel_bed * b.resin_diam) / prop_in.visc_k_phase['Liq'])

        @self.Constraint(ion_set, doc='Schmidt number')

        def eq_Sc(b, j):
            prop_in = b.properties_in[0]
            return (b.Sc[j] == prop_in.visc_k_phase['Liq'] / prop_in.diffus_phase_comp['Liq', j])
        ...
        #                 (pyunits.convert(b.resin_eq_capacity, to_units=pyunits.mol/pyunits.m**3)))
        ...
        def eq_fluid_mass_transfer_coeff(b, j):
            prop_in = b.properties_in[0]
            return (b.fluid_mass_transfer_coeff[j] == (prop_in.diffus_phase_comp['Liq', j] * b.Sh[j]) / b.resin_diam)

        @self.Constraint(ion_set, doc='Rate coefficient')

        def eq_rate_coeff(b, j):
            return b.rate_coeff[j] == (6 * (1 - b.bed_porosity) * b.fluid_mass_transfer_coeff[j]) / \
                (pyunits.convert(b.resin_bulk_dens, to_units=pyunits.kg/pyunits.m**3) * b.resin_diam)

        @self.Constraint(ion_set, doc='HTU')

        def eq_HTU(b, j):
            return b.HTU[j] == b.vel_bed / (pyunits.convert(b.resin_bulk_dens, to_units=pyunits.kg/pyunits.m**3) * b.rate_coeff[j])

        @self.Constraint(doc='Partition ratio')

        def eq_partition_ratio(b):
            prop_in = b.properties_in[0]
            return b.partition_ratio == (b.resin_eq_capacity) / \
                pyunits.convert(sum(prop_in.conc_equiv_phase_comp['Liq', j] for j in ion_set), to_units=pyunits.mol/pyunits.L)
        ...
        def eq_dimensionless_time(b):
            return (b.dimensionless_time == ((b.vel_bed * b.t_breakthru) / b.bed_depth - b.bed_porosity) / b.partition_ratio)

        @self.Constraint(ion_set,
            doc='Number of mass-transfer unts')

        def eq_num_transfer_units(b, j):
            return b.num_transfer_units == (b.fluid_mass_transfer_coeff[j] * b.resin_surf_per_vol * b.bed_depth) / b.vel_bed
        ...
        def eq_flow_waste(b):
            ...
            return (prop_waste.flow_vol_phase['Liq'] == 0.05 * prop_in.flow_vol_phase['Liq'])
        ...
        def eq_mass_out(b, j):
            prop_in = b.properties_in[0]
            return (b.mass_out[j] == prop_in.flow_vol_phase['Liq'] * prop_in.conc_equiv_phase_comp['Liq', j] * b.t_breakthru - \
                pyunits.convert(b.resin_eq_capacity, to_units=pyunits.mol/pyunits.m**3) * (b.bed_depth * (b.bed_diam / 2) ** 2 ))
        ...
        def eq_ss_effluent(b, j):
            ...
            return (prop_out.conc_equiv_phase_comp['Liq', j] == b.mass_out[j] / \
                (prop_in.flow_vol_phase['Liq'] * prop_in.conc_equiv_phase_comp['Liq', j] * b.t_breakthru))
    ...
    def calculate_scaling_factors(self):
        ...
        # iscale.set_scaling_factor(
        #     self.fluid_mass_transfer_coeff, 1E5
        # )  
    ...
    def _get_performance_contents(self, time_point=0):
        var_dict = {}
        # var_dict["Breakthrough Time [hr]"] = pyunits.convert(self.t_breakthru, to_units=pyunits.hr)
        # var_dict["Breakthrough Time [d]"] = pyunits.convert(self.t_breakthru, to_units=pyunits.day)
        # var_dict["Contact Time [min]"] = pyunits.convert(self.t_contact, to_units=pyunits.min)
        # var_dict["Toal Resin Capacity [eq/L]"] = self.resin_max_capacity
        # var_dict["Usable Resin Capacity [eq/L]"] = self.resin_eq_capacity
        # var_dict["Resin Particle Diameter [cm]"] = pyunits.convert(self.resin_diam, to_units=pyunits.cm)
        # var_dict["Resin Bulk Density [kg/L]"] = self.resin_bulk_dens
        # var_dict["Resin Particle Density [kg/L]"] = self.resin_particle_dens
        ...
        #     # kf = f'Fluid Mass Transfer Coeff. for {ion} [m/s]'
        #     # kd = f'Rate Coeff. for {ion} [m3/(kg*s)]'
        ...
        # var_dict["Bed Depth [m]"] = self.bed_depth
        # var_dict["Bed Diameter [m]"] = self.bed_diam
        # var_dict["Bed Volume [m3]"] = self.bed_depth * self.pi() * (self.bed_diam / 2) ** 2
        ...
        #     var_dict[kf] = self.fluid_mass_transfer_coeff[i]
        #     var_dict[kd] = self.rate_coeff[i]
        # var_dict["Minimum bed velocity [m/hr]"] = pyunits.convert(self.vel_min, to_units=pyunits.m/pyunits.hr)
        # var_dict["Bed velocity [m/hr]"] = pyunits.convert(self.vel_bed, to_units=pyunits.m/pyunits.hr)
