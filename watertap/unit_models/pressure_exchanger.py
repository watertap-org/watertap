...
from pyomo.environ import Block, Var, Suffix, NonNegativeReals, Reals, value, units as pyunits
...
class PressureExchangerData(UnitModelBlockData):
    ...
    **MomentumBalanceType.momentumPhase** - momentum balances for each phase.}""",
    ...
    def build(self):
        ...
        self.low_pressure_side.add_material_balances(balance_type=self.config.material_balance_type)
        ...
        @self.low_pressure_side.Expression(
            self.flowsheet().config.time, doc="Work transferred to low pressure side fluid"
        )
        def work(b, t):
            return b.properties_in[t].flow_vol * b.deltaP[t]
        ...
        @self.Constraint(self.flowsheet().config.time, doc="Equal low pressure on both sides")
    ...
    def initialize(
        self, state_args=None, routine=None, outlvl=idaeslog.NOTSET, solver=None, optarg=None
    ):
        """
        ...
        # low pressure side
        propogate_state(
            self.low_pressure_side.properties_in[0], self.low_pressure_side.properties_out[0]
        )
        self.low_pressure_side.properties_out[0].pressure = self.low_pressure_side.properties_in[
            0
        ...
        # high pressure side
        propogate_state(
            self.high_pressure_side.properties_in[0], self.high_pressure_side.properties_out[0]
        )
