    ...
    **Valid values:** {
    ...
    def build(self):
        ...
        def flux_mass_phase_comp_avg(b, t, p, j):
            return 0.5 * (
                b.flux_mass_phase_comp_in[t, p, j] + b.flux_mass_phase_comp_out[t, p, j]
            )
        ...
        def eq_flux_in(b, t, p, j):
            ...
            if comp.is_solvent():
                return b.flux_mass_phase_comp_in[t, p, j] == b.A_comp[
                    t, j
                ] * b.dens_solvent * (
                    (prop_feed.pressure - prop_perm.pressure)
                    - b.sigma[t]
                    * (prop_feed.pressure_osm_phase[p] - prop_perm.pressure_osm_phase[p])
                )
            elif comp.is_solute():
        ...
        def eq_flux_out(b, t, p, j):
            ...
            comp = self.config.property_package.get_component(j)
            if comp.is_solvent():
                return b.flux_mass_phase_comp_out[t, p, j] == b.A_comp[
                    t, j
                ] * b.dens_solvent * (
                    (prop_feed.pressure - prop_perm.pressure)
                    - b.sigma[t]
    ...
    def calculate_scaling_factors(self):
                    ...
                    if comp.is_solvent():  # scaling based on solvent flux equation
                        sf = (
                            ...
                            * iscale.get_scaling_factor(
                                self.feed_side.properties_in[t].pressure
                            )
                        )
        ...
        for vobj in [
            self.avg_conc_mass_phase_comp_in,
            self.avg_conc_mass_phase_comp_out,
        ]:
            for (t, p, j), v in vobj.items():
                if iscale.get_scaling_factor(v) is None:
                    sf = iscale.get_scaling_factor(
