...
class TestPressureExchanger:
    ...
    def unit_frame(self):
        ...
        m.fs.unit.low_pressure_side.properties_in[0].mass_frac_phase_comp["Liq", "TDS"].fix(
            lowP_mass_frac_TDS
        )
        ...
        m.fs.unit.high_pressure_side.properties_in[0].mass_frac_phase_comp["Liq", "TDS"].fix(
            highP_mass_frac_TDS
        )
    ...
    def test_initialize(self, unit_frame):
        m = unit_frame
        kwargs = {"solver": "ipopt", "optarg": {"bound_push": 1e-8}}
    ...
    def test_solution(self, unit_frame):
        ...
        assert pytest.approx(4.755e6, rel=1e-3) == value(m.fs.unit.low_pressure_outlet.pressure[0])
        ...
        assert pytest.approx(4.654e6, rel=1e-3) == value(m.fs.unit.low_pressure_side.deltaP[0])
        assert pytest.approx(4.654e3, rel=1e-3) == value(m.fs.unit.low_pressure_side.work[0])
        assert pytest.approx(-4.899e6, rel=1e-3) == value(m.fs.unit.high_pressure_side.deltaP[0])
        assert pytest.approx(-4.899e3, rel=1e-3) == value(m.fs.unit.high_pressure_side.work[0])
