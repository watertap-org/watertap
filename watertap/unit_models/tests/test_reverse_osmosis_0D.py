...
def test_config():
    ...
    assert m.fs.unit.config.mass_transfer_coefficient == MassTransferCoefficient.calculated
...
def test_option_concentration_polarization_type_fixed():
    ...
    assert m.fs.unit.config.concentration_polarization_type == ConcentrationPolarizationType.fixed
...
def test_option_pressure_change_calculated():
    ...
    assert m.fs.unit.config.concentration_polarization_type == ConcentrationPolarizationType.none
...
class TestReverseOsmosis:
    ...
    def test_build(self, RO_frame):
        ...
        cv_stateblock_lst = [
            "properties_in",
            "properties_out",
            "properties_interface_in",
            "properties_interface_out",
        ]
        ...
        cv_objs_type_dict = {
            "eq_concentration_polarization_io": Constraint,
    ...
    def test_conservation(self, RO_frame):
        ...
        assert abs(value(flow_mass_inlet - flow_mass_retentate - flow_mass_permeate)) <= 1e-5
        ...
        assert (
            abs(
                value(
                    ...
                    - flow_mass_permeate
                    * b.permeate_side.properties_mixed[0].enth_mass_phase["Liq"]
                )
            )
            <= 1e-5
        )
    ...
    def test_solution(self, RO_frame):
        ...
        assert pytest.approx(0.2361, rel=1e-3) == value(
            m.fs.unit.permeate_side.properties_mixed[0].flow_mass_phase_comp["Liq", "H2O"]
        )
        assert pytest.approx(7.879e-5, rel=1e-3) == value(
            m.fs.unit.permeate_side.properties_mixed[0].flow_mass_phase_comp["Liq", "NaCl"]
        )
        assert pytest.approx(value(m.fs.unit.cp_modulus[0, "NaCl"]), rel=1e-3) == value(
            m.fs.unit.feed_side.properties_interface_in[0].conc_mass_phase_comp["Liq", "NaCl"]
        ) / value(m.fs.unit.feed_side.properties_in[0].conc_mass_phase_comp["Liq", "NaCl"])
        assert pytest.approx(value(m.fs.unit.cp_modulus[0, "NaCl"]), rel=1e-3) == value(
            m.fs.unit.feed_side.properties_interface_out[0].conc_mass_phase_comp["Liq", "NaCl"]
