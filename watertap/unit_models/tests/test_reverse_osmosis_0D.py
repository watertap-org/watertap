...
class TestReverseOsmosis:
    ...
    def test_conservation(self, RO_frame):
        ...
        flow_mass_permeate = sum(
            b.permeate_side.properties_mixed[0].flow_mass_phase_comp["Liq", j]
            for j in comp_lst
        )
    ...
    def test_solution(self, RO_frame):
        ...
        assert pytest.approx(0.2361, rel=1e-3) == value(
            m.fs.unit.permeate_side.properties_mixed[0].flow_mass_phase_comp[
                "Liq", "H2O"
            ]
        )
        ...
        assert pytest.approx(value(m.fs.unit.cp_modulus[0, "NaCl"]), rel=1e-3) == value(
            m.fs.unit.feed_side.properties_interface_in[0].conc_mass_phase_comp[
                "Liq", "NaCl"
            ]
        ) / value(
        ...
        assert pytest.approx(value(m.fs.unit.cp_modulus[0, "NaCl"]), rel=1e-3) == value(
            m.fs.unit.feed_side.properties_interface_out[0].conc_mass_phase_comp[
                "Liq", "NaCl"
            ]
        ) / value(
