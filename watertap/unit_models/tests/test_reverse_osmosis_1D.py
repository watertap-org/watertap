...
def test_config():
    ...
    assert (
        m.fs.unit.config.concentration_polarization_type is ConcentrationPolarizationType.calculated
    )
...
class TestReverseOsmosis:
    ...
    def testReverseOsmosis_cp_calculated_kf_calculated(self):
        ...
        set_scaling_factor(
            m.fs.unit.feed_side.properties[0, 1].flow_mass_phase_comp["Liq", "NaCl"], 1e4
        )
        calculate_scaling_factors(m)
