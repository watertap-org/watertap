...
import pytest
from pyomo.environ import ConcreteModel, value, Var, Constraint, assert_optimal_termination
...
class TestNanofiltration:
    ...
    def unit_frame(self):
        ...
        m.fs.unit.eq_electroneutrality = Constraint(
            expr=0
            == sum(
                charge_comp[j] * m.fs.unit.feed_side.properties_out[0].conc_mol_phase_comp["Liq", j]
    ...
    def test_solution(self, unit_frame):
        ...
        assert pytest.approx(0.86, rel=1e-3) == value(m.fs.unit.recovery_vol_phase[0, "Liq"])
