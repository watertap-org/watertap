...
import pytest
from pyomo.environ import ConcreteModel, TerminationCondition, SolverStatus, value, Constraint, Var
...
class TestPumpIsothermal:
    ...
    def test_conservation(self, Pump_frame):
        ...
        for t in m.fs.config.time:
            ...
            # energy balance
            assert abs(value(b.properties_in[t].enth_flow - b.properties_out[t].enth_flow)) <= 1e-6
    ...
    def test_solution(self, Pump_frame):
        ...
        assert pytest.approx(0.035, rel=1e-3) == value(
            m.fs.unit.control_volume.properties_out[0].flow_mass_phase_comp["Liq", "TDS"]
        )
