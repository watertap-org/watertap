...
from watertap.unit_models.chemical_precipitator import ChemicalPrecipitator
...
class TestChemicalPrecipitator:
    ...
    def basic_unit_mass(self):
        ...
        m.fs.unit = ChemicalPrecipitator(
    ...
    def test_molar(self, basic_unit_molar):
        ...
        flow_ca_in_reagent = (
            1e-3
            / (56.0774 * 1e-3)
            * value(m.fs.unit.reactor.properties_in[0].flow_vol_phase["Liq"])
        )
        ...
        assert pytest.approx(expected_ca_mol_flow, rel=1e-5) == value(
            m.fs.unit.reactor.properties_out[0].flow_mol_phase_comp["Liq", "Ca_2+"]
        )
        ...
        assert pytest.approx(expected_na_mol_flow, rel=1e-5) == value(
            m.fs.unit.reactor.properties_out[0].flow_mol_phase_comp["Liq", "Na_+"]
        )

        expected_mg_mol_flow = 0.1 - 1e-4 / (58.3197 * 1e-3)

        assert pytest.approx(expected_mg_mol_flow, rel=1e-5) == value(
            m.fs.unit.reactor.properties_out[0].flow_mol_phase_comp["Liq", "Mg_2+"]
        )
    ...
    def test_mass(self, basic_unit_mass):
        ...
        flow_ca_in_reagent = 1e-3 * value(
            m.fs.unit.reactor.properties_in[0].flow_vol_phase["Liq"]
        )
        expected_ca_flow = 0.001 + flow_ca_in_reagent / 56.0774 * 40 - 1e-3 / 100.9 * 40
        ...
        assert pytest.approx(expected_ca_flow, rel=1e-2) == value(
            m.fs.unit.reactor.properties_out[0].flow_mass_phase_comp["Liq", "Ca_2+"]
        )
        ...
        assert pytest.approx(expected_na_flow, rel=1e-5) == value(
            m.fs.unit.reactor.properties_out[0].flow_mass_phase_comp["Liq", "Na_+"]
        )

        expected_mg_flow = 0.001 - 1e-4 / 58.3197 * 24.305

        assert pytest.approx(expected_mg_flow, rel=1e-5) == value(
            m.fs.unit.reactor.properties_out[0].flow_mass_phase_comp["Liq", "Mg_2+"]
        )
    ...
    def test_costing(self, basic_unit_molar):
        ...
        assert value(m.fs.costing.chemical_precipitator.capital_cost_param) == 2000
        assert (
