...
class TestBoronRemoval_IonPropPack_Min(UnitTestHarness):
    ...
    @pytest.mark.unit
    def test_units_consistent(self, base_boron_removal_model):
        m = base_boron_removal_model
        assert_units_consistent(m)

    @pytest.mark.unit
    def test_dof(self, base_boron_removal_model):
        m = base_boron_removal_model
        # set the variables
        state = {
            "H2O": 100,
        ...
        if degrees_of_freedom(m) != 0:
            raise UnitAttributeError(
                ...
                "".format(dof=degrees_of_freedom(m))
            )
    ...
    @pytest.mark.component
    def test_reporting(self, min_boron_removal_model):
        m = min_boron_removal_model
        m.fs.unit.report()
...
class TestBoronRemoval_IonPropPack_with_ResAlk(UnitTestHarness):
    ...
    @pytest.mark.unit
    def test_units_consistent(self, alk_boron_removal_model):
        m = alk_boron_removal_model
        assert_units_consistent(m)

    @pytest.mark.unit
    def test_dof(self, alk_boron_removal_model):
        m = alk_boron_removal_model
        # set the variables
