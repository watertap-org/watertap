...
class TestFeedWaterTankZO:
    ...
    def model(self):
        ...
        m.fs.unit = FeedWaterTankZO(property_package=m.fs.params, database=m.db)
...
def test_costing():
    ...
    m.fs.unit1 = FeedWaterTankZO(property_package=m.fs.params, database=m.db)
    ...
    assert isinstance(m.fs.costing.feed_water_tank.capital_a_parameter, Var)
    assert isinstance(m.fs.costing.feed_water_tank.capital_b_parameter, Var)
