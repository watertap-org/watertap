...
class TestDualMediaFiltrationZO:
    ...
    def model(cls):
        ...
        m.fs.params = WaterParameterBlock(solute_list=["nonvolatile_toc", "toc", "tss"])
    ...
    def test_solution(self, model):
        ...
        assert pytest.approx(0.01153, rel=1e-5) == value(
            model.fs.unit.properties_treated[0].flow_vol
        )
        assert pytest.approx(69.384, rel=1e-5) == value(
            model.fs.unit.properties_treated[0].conc_mass_comp["nonvolatile_toc"]
        )
        assert pytest.approx(69.384, rel=1e-5) == value(
            model.fs.unit.properties_treated[0].conc_mass_comp["toc"]
        )
        assert pytest.approx(2.6019, rel=1e-5) == value(
            model.fs.unit.properties_treated[0].conc_mass_comp["tss"]
        )
        assert pytest.approx(1.47e-3, rel=1e-5) == value(
            model.fs.unit.properties_byproduct[0].flow_vol
        )
        assert pytest.approx(136.054, rel=1e-5) == value(
        ...
        assert pytest.approx(659.864, rel=1e-5) == value(
            model.fs.unit.properties_byproduct[0].conc_mass_comp["tss"]
        )
        assert pytest.approx(2.37363, abs=1e-5) == value(model.fs.unit.electricity[0])
...
class TestDualMediaFiltrationZO_w_default_removal:
    ...
    def test_solution(self, model):
        ...
        assert pytest.approx(0.01253, rel=1e-5) == value(
            model.fs.unit.properties_treated[0].flow_vol
        )
        assert pytest.approx(63.8468, rel=1e-5) == value(
            model.fs.unit.properties_treated[0].conc_mass_comp["nonvolatile_toc"]
        )
        assert pytest.approx(63.8468, rel=1e-5) == value(
            model.fs.unit.properties_treated[0].conc_mass_comp["toc"]
        )
        assert pytest.approx(2.39425, rel=1e-5) == value(
            model.fs.unit.properties_treated[0].conc_mass_comp["tss"]
        )
        assert pytest.approx(79.8085, rel=1e-5) == value(
        ...
        assert pytest.approx(2.55621, abs=1e-5) == value(model.fs.unit.electricity[0])
...
def test_costing():
    ...
    assert isinstance(m.fs.costing.dual_media_filtration, Block)
    assert isinstance(m.fs.costing.dual_media_filtration.capital_a_parameter, Var)
    assert isinstance(m.fs.costing.dual_media_filtration.capital_b_parameter, Var)
    assert isinstance(m.fs.costing.dual_media_filtration.reference_state, Var)
    ...
    assert (
        ...
        == 3828050.12  # equal to basis capital cost
    )
    assert pytest.approx(value(m.fs.costing.total_capital_cost), rel=1e-3) == 6641571.25
    assert pytest.approx(value(m.fs.costing.LCOW), rel=1e-3) == 0.015489
    assert pytest.approx(value(m.fs.costing.SEC), rel=1e-3) == 0.05071
