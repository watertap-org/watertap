...
class TestUltraFiltrationZO_w_default_removal:
    ...
    def test_solution(self, model):
        ...
        assert pytest.approx(1.38592e-8, rel=1e-5) == value(
...
def test_costing():
    ...
    assert (
        ...
        == 509912.23  # costing reference_state
    )
    assert pytest.approx(value(m.fs.costing.LCOW), rel=1e-3) == 0.043178
    assert pytest.approx(value(m.fs.costing.SEC), rel=1e-3) == 0.23134
