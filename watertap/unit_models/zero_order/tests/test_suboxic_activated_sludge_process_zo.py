...
from watertap.unit_models.zero_order import SuboxicASMZO
...
class TestSuboxicASMZO_w_default_removal:
    ...
    def model(self):
        ...
        m.fs.unit.inlet.flow_mass_comp[0, "tkn"].fix(1)
    ...
    def test_solution(self, model):
        ...
        assert pytest.approx(0.0113288, rel=1e-5) == value(
            model.fs.unit.properties_treated[0].flow_vol
        )
        assert pytest.approx(10.1864, rel=1e-5) == value(
            model.fs.unit.properties_treated[0].conc_mass_comp["tkn"]
        )
        assert pytest.approx(10.1864, rel=1e-5) == value(
            model.fs.unit.properties_treated[0].conc_mass_comp["tkn"]
        )
        assert pytest.approx(3.6712e-3, rel=1e-5) == value(
            model.fs.unit.properties_byproduct[0].flow_vol
        )
        assert pytest.approx(268.332, rel=1e-5) == value(
            model.fs.unit.properties_byproduct[0].conc_mass_comp["tss"]
        )
        assert pytest.approx(240.957, rel=1e-5) == value(
            model.fs.unit.properties_byproduct[0].conc_mass_comp["tkn"]
        )
        assert pytest.approx(27, abs=1e-5) == value(model.fs.unit.electricity[0])
...
def test_costing():
    ...
    m.fs.params = ZOParameterBlock(solute_list=["bod", "tss", "tkn", "phosphorus"])
    ...
    m.fs.unit1.inlet.flow_mass_comp[0, "tkn"].fix(1)
    m.fs.unit1.inlet.flow_mass_comp[0, "phosphorus"].fix(1)
