...
class TestCoolingSupplyZO:
    ...
    def model(self):
        ...
        m.fs.unit = CoolingSupplyZO(default={
...
def test_costing():
    ...
    m.fs.unit1 = CoolingSupplyZO(default={
        "property_package": m.fs.params,
        "database": m.db})
    ...
    assert isinstance(m.fs.costing.cooling_supply.capital_a_parameter,
                      Var)
    assert isinstance(m.fs.costing.cooling_supply.capital_b_parameter,
                      Var)
