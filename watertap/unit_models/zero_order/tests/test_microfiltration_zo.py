...
def test_costing():
    ...
    m.fs.unit1 = MicroFiltrationZO(default={
        "property_package": m.fs.params,
        "database": m.db})
    ...
    assert isinstance(m.fs.costing.microfiltration.capital_a_parameter,
                      Var)
    assert isinstance(m.fs.costing.microfiltration.capital_b_parameter,
                      Var)
