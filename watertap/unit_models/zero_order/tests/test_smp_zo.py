...
from pyomo.environ import (
    Block, check_optimal_termination, ConcreteModel, Constraint, value, Var, assert_optimal_termination)
...
from watertap.unit_models.zero_order import SMPZO
...
class TestSMPZO:
    ...
    def test_load_parameters(self, model):
        data = model.db.get_unit_operation_parameters("smp")
    ...
    def test_solution(self, model):
        ...
        assert (pytest.approx(7e-10, rel=1e-5) ==
...
def test_costing():
    ...
    assert isinstance(m.fs.costing.smp, Block)
    assert isinstance(m.fs.costing.smp.capital_a_parameter,
                      Var)
    assert isinstance(m.fs.costing.smp.capital_b_parameter,
                      Var)
    assert isinstance(m.fs.costing.smp.reference_state, Var)
