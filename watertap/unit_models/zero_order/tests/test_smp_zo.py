...
class TestSMPZO:
    ...
    def model(self):
        ...
        m.fs.unit = SMPZO(default={"property_package": m.fs.params, "database": m.db})
...
def test_costing():
    ...
    m.fs.unit1 = SMPZO(default={"property_package": m.fs.params, "database": m.db})
    ...
    assert isinstance(m.fs.costing.smp.capital_a_parameter, Var)
    assert isinstance(m.fs.costing.smp.capital_b_parameter, Var)
