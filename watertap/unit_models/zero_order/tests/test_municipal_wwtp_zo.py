...
class TestMunicipalWWTPZO:
    ...
    def test_load_parameters(self, model):
        data = model.db.get_unit_operation_parameters("municipal_wwtp")
...
def test_costing():
    ...
    m.fs.unit1 = MunicipalWWTPZO(default={
        "property_package": m.fs.params,
        "database": m.db})
    ...
    assert isinstance(m.fs.costing.municipal_wwtp.capital_a_parameter,
                      Var)
    assert isinstance(m.fs.costing.municipal_wwtp.capital_b_parameter,
                      Var)
    assert isinstance(m.fs.costing.municipal_wwtp.reference_state, Var)
