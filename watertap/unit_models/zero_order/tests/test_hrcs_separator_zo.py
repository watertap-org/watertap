...
###############################################################################
"""
Tests for zero-order high-rate contact stabilization (HR-CS) separator model
"""
import pytest
...
from watertap.unit_models.zero_order import HRCSSeparatorZO
...
class TestHRCSSeparatorZO:
    ...
    def model(self):
        ...
        m.fs.unit = HRCSSeparatorZO(property_package=m.fs.params, database=m.db)
    ...
    def test_build(self, model):
        ...
        assert model.fs.unit._tech_type == "hrcs_separator"
    ...
    def test_load_parameters(self, model):
        data = model.db.get_unit_operation_parameters("hrcs_separator")
    ...
    def test_solution(self, model):
        ...
        assert pytest.approx(0.50015, rel=1e-5) == value(
        ...
        assert pytest.approx(1.9994e-8, rel=1e-5) == value(
            model.fs.unit.properties_treated[0].conc_mass_comp["carbon_dioxide"]
        )
        assert pytest.approx(9.5029, rel=1e-5) == value(
        ...
        assert pytest.approx(1.0523e-9, rel=1e-5) == value(
...
class TestHRCSSeparatorZO_w_default_removal:
    @pytest.fixture(scope="class")
    def model(self):
        m = ConcreteModel()
        m.db = Database()
    ...
    def test_solution(self, model):
        ...
        assert pytest.approx(0.50115, rel=1e-5) == value(
            model.fs.unit.properties_treated[0].flow_vol
        )
        assert pytest.approx(0.099771, rel=1e-5) == value(
            model.fs.unit.properties_treated[0].conc_mass_comp["tss"]
        )
        assert pytest.approx(0.099771, rel=1e-5) == value(
            model.fs.unit.properties_treated[0].conc_mass_comp["cod"]
        ...
        assert pytest.approx(1.9954e-8, rel=1e-5) == value(
            model.fs.unit.properties_treated[0].conc_mass_comp["carbon_dioxide"]
        )
        assert pytest.approx(1.9954, rel=1e-5) == value(
...
def test_costing():
    ...
    assert isinstance(m.fs.costing.hrcs_separator, Block)
    assert isinstance(m.fs.costing.hrcs_separator.HRT, Var)
    assert isinstance(m.fs.costing.hrcs_separator.sizing_cost, Var)
