...
class TestNFZO:
    ...
    def test_load_parameters(self, model):
        ...
        assert model.fs.unit.recovery_vol[0].value == data["recovery_vol"]["value"]
        ...
        assert (
            model.fs.unit.electricity_intensity.value
            == data["electricity_intensity"]["value"]
        )
    ...
    def test_solution(self, model):
        assert pytest.approx(8.5, rel=1e-5) == value(model.fs.unit.treated.flow_vol[0])
        assert pytest.approx(0.0352941, rel=1e-5) == value(
            model.fs.unit.treated.conc_mass_comp[0, "sulfur"]
        )
        assert pytest.approx(0.588235, rel=1e-5) == value(
            model.fs.unit.treated.conc_mass_comp[0, "toc"]
        )
        assert pytest.approx(0.105882, rel=1e-5) == value(
            model.fs.unit.treated.conc_mass_comp[0, "tss"]
        )

        assert pytest.approx(1.5, rel=1e-5) == value(
            model.fs.unit.byproduct.flow_vol[0]
        )
        assert pytest.approx(6.46666, rel=1e-5) == value(
            model.fs.unit.byproduct.conc_mass_comp[0, "sulfur"]
        )
        assert pytest.approx(10, rel=1e-5) == value(
            model.fs.unit.byproduct.conc_mass_comp[0, "toc"]
        )
        assert pytest.approx(19.4, rel=1e-5) == value(
            model.fs.unit.byproduct.conc_mass_comp[0, "tss"]
        )

        assert pytest.approx(10 * 0.231344952 * 3600, rel=1e-5) == value(
...
class TestNFZO_w_default_removal:
    ...
    def test_solution(self, model):
        ...
        assert pytest.approx(4.70588, rel=1e-5) == value(
            model.fs.unit.treated.conc_mass_comp[0, "foo"]
        )
        ...
        assert pytest.approx(0, abs=1e-5) == value(
            model.fs.unit.byproduct.conc_mass_comp[0, "foo"]
        )
