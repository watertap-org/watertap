...
class TestWaterPumpingStationZO:
    ...
    def model(self):
        ...
        m.fs.unit = WaterPumpingStationZO(default={"property_package": m.fs.params,
                                                   "database": m.db})
    ...
    def test_load_parameters(self, model):
        ...
        assert model.fs.unit.electricity[0].value == \
               data["electricity"]["value"]

        assert model.fs.unit.eta_motor[0].fixed
        assert model.fs.unit.eta_motor[0].value == data[
            "eta_motor"]["value"]
    ...
    def test_solution(self, model):
        ...
        assert (pytest.approx(93.21, rel=1e-5) ==
                value(model.fs.unit.electricity[0]))
        assert (pytest.approx(25.27007, rel=1e-5) ==
                value(model.fs.unit.lift_height[0]))
...
class TestWaterPumpingStationZO_without_fix_pump_power_config:
    ...
    def test_load_parameters(self, model):
        ...
        assert model.fs.unit.lift_height[0].value == \
               data["lift_height"]["value"]
...
class TestPumpZOsubtype:
    ...
    def test_load_parameters(self, model, subtype):
        ...
        data = db.get_unit_operation_parameters("water_pumping_station", subtype=subtype)
...
def test_costing(subtype):
    ...
    m.fs.unit1 = WaterPumpingStationZO(default={
        "property_package": m.fs.params,
        "database": m.db,
