...
class TestMagprexZO:
    ...
    def model(self):
        ...
        m.fs.params = ZOParameterBlock(solute_list=["phosphates", "struvite"])
...
class TestMagprexZO_w_default_removal:
    ...
    def model(self):
        ...
        m.fs = FlowsheetBlock(dynamic=False)
        m.fs.params = ZOParameterBlock(solute_list=["phosphates", "struvite", "foo"])
    ...
    def test_solution(self, model):
        ...
        assert pytest.approx(2.79904e-08, rel=1e-5) == value(
            model.fs.unit.properties_treated[0].conc_mass_comp["struvite"]
        )
