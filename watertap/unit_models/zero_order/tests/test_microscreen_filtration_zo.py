...
class TestMicroscreenFiltrationZO:
    ...
    def model(self):
        ...
        m.fs.unit = MicroscreenFiltrationZO(default={
    ...
    def test_solution(self, model):
        ...
        assert (pytest.approx(47.6195, rel=1e-5) ==
                value(model.fs.unit.properties_treated[0].conc_mass_comp["tss"]))
        assert (pytest.approx(5.001e-4, rel=1e-5) ==
                value(model.fs.unit.properties_byproduct[0].flow_vol))
        assert (pytest.approx(999.800, rel=1e-5) ==
...
class TestMicroscreenFiltrationZO_w_default_removal:
    ...
    def test_solution(self, model):
        ...
        assert (pytest.approx(0.0115, rel=1e-5) ==
                value(model.fs.unit.properties_treated[0].flow_vol))
        assert (pytest.approx(43.4786, rel=1e-5) ==
                value(model.fs.unit.properties_treated[0].conc_mass_comp["tss"]))
        assert (pytest.approx(86.9573, rel=1e-5) ==
                value(model.fs.unit.properties_treated[0].conc_mass_comp["foo"]))
        assert (pytest.approx(5.001e-4, rel=1e-5) ==
                value(model.fs.unit.properties_byproduct[0].flow_vol))
        assert (pytest.approx(999.800, rel=1e-5) ==
                value(model.fs.unit.properties_byproduct[0].conc_mass_comp["tss"]))
        assert (pytest.approx(1.59968e-6, rel=1e-5) ==
...
def test_costing():
    ...
    m.fs.unit1 = MicroscreenFiltrationZO(default={
        "property_package": m.fs.params,
        "database": m.db})
    ...
    assert isinstance(m.fs.costing.microscreen_filtration.capital_a_parameter,
                      Var)
    assert isinstance(m.fs.costing.microscreen_filtration.capital_b_parameter,
                      Var)
