...
class TestEvaporationPondZO:
    ...
    def model(self):
        ...
        m.fs.unit = EvaporationPondZO(default={"property_package": m.fs.params,
                                     "database": m.db})
    ...
    def test_load_parameters(self, model):
        ...
        assert model.fs.unit.evaporation_rate_adj_factor[0].value == data[
            "evaporation_rate_adj_factor"]["value"]
        assert model.fs.unit.evap_rate_calc_a_parameter[0].fixed
        assert model.fs.unit.evap_rate_calc_a_parameter[0].value == data[
            "evap_rate_calc_a_parameter"]["value"]
        assert model.fs.unit.evap_rate_calc_b_parameter[0].fixed
        assert model.fs.unit.evap_rate_calc_b_parameter[0].value == data[
            "evap_rate_calc_b_parameter"]["value"]
        assert model.fs.unit.evap_rate_calc_c_parameter[0].fixed
        assert model.fs.unit.evap_rate_calc_c_parameter[0].value == data[
            "evap_rate_calc_c_parameter"]["value"]
        assert model.fs.unit.adj_area_calc_a_parameter[0].fixed
        assert model.fs.unit.adj_area_calc_a_parameter[0].value == data[
            "adj_area_calc_a_parameter"]["value"]
        assert model.fs.unit.adj_area_calc_b_parameter[0].fixed
        assert model.fs.unit.adj_area_calc_b_parameter[0].value == data[
            "adj_area_calc_b_parameter"]["value"]
    ...
    def test_solution(self, model):
        assert (pytest.approx(1.411, rel=1e-5) ==
                value(model.fs.unit.properties_in[0].flow_vol))
        assert (pytest.approx(7.087172, rel=1e-5) ==
                value(model.fs.unit.properties_in[0].conc_mass_comp["H2O"]))
        assert (pytest.approx(87.17221, rel=1e-5) ==
                value(model.fs.unit.properties_in[0].conc_mass_comp["tds"]))
        assert (pytest.approx(323.175053, rel=1e-5) ==
                value(model.fs.unit.properties_in[0].conc_mass_comp["magnesium"]))
        assert (pytest.approx(559.177888, rel=1e-5) ==
                value(model.fs.unit.properties_in[0].conc_mass_comp["calcium"]))
        assert (pytest.approx(7.087172, rel=1e-5) ==
                value(model.fs.unit.properties_in[0].conc_mass_comp["nitrate"]))
        assert (pytest.approx(7.795889, rel=1e-5) ==
                value(model.fs.unit.properties_in[0].conc_mass_comp["sulfate"]))
        assert (pytest.approx(8.504606, rel=1e-5) ==
                value(model.fs.unit.properties_in[0].conc_mass_comp["tss"]))

        assert (pytest.approx(0.028021, rel=1e-5) ==
                value(model.fs.unit.properties_treated[0].flow_vol))
        assert (pytest.approx(0.0356878, rel=1e-5) ==
                value(model.fs.unit.properties_treated[0].conc_mass_comp["H2O"]))
        assert (pytest.approx(87.791299, rel=1e-5) ==
                value(model.fs.unit.properties_treated[0].conc_mass_comp["tds"]))
        assert (pytest.approx(325.470182, rel=1e-5) ==
                value(model.fs.unit.properties_treated[0].conc_mass_comp["magnesium"]))
        assert (pytest.approx(563.149066, rel=1e-5) ==
                value(model.fs.unit.properties_treated[0].conc_mass_comp["calcium"]))
        assert (pytest.approx(7.13750, rel=1e-5) ==
                value(model.fs.unit.properties_treated[0].conc_mass_comp["nitrate"]))
        assert (pytest.approx(7.8512544, rel=1e-5) ==
                value(model.fs.unit.properties_treated[0].conc_mass_comp["sulfate"]))
        assert (pytest.approx(8.5650048, rel=1e-5) ==
                value(model.fs.unit.properties_treated[0].conc_mass_comp["tss"]))

        assert (pytest.approx(1.382979, rel=1e-5) ==
                value(model.fs.unit.properties_byproduct[0].flow_vol))
        assert (pytest.approx(7.2300447, rel=1e-5) ==
                value(model.fs.unit.properties_byproduct[0].conc_mass_comp["H2O"]))
        assert (pytest.approx(87.15967, rel=1e-5) ==
                value(model.fs.unit.properties_byproduct[0].conc_mass_comp["tds"]))
        assert (pytest.approx(323.128550, rel=1e-5) ==
                value(model.fs.unit.properties_byproduct[0].conc_mass_comp["magnesium"]))
        assert (pytest.approx(559.097426, rel=1e-5) ==
                value(model.fs.unit.properties_byproduct[0].conc_mass_comp["calcium"]))
        assert (pytest.approx(7.086152, rel=1e-5) ==
                value(model.fs.unit.properties_byproduct[0].conc_mass_comp["nitrate"]))
        assert (pytest.approx(7.794767, rel=1e-5) ==
                value(model.fs.unit.properties_byproduct[0].conc_mass_comp["sulfate"]))
        assert (pytest.approx(8.503382, rel=1e-5) ==
