...
class TestSettlingPondZO:
    ...
    def model(self):
        ...
        m.fs.unit = SettlingPondZO(default={
    ...
    def test_solution(self, model):
        ...
        assert (pytest.approx(90.909, rel=1e-5) ==
                value(model.fs.unit.properties_in[0].conc_mass_comp["tss"]))
        assert (pytest.approx(0.010008174, rel=1e-5) ==
                value(model.fs.unit.properties_treated[0].flow_vol))
        assert (pytest.approx(0.91665, rel=1e-5) ==
                value(model.fs.unit.properties_treated[0].conc_mass_comp["tss"]))
        assert (pytest.approx(0.00099183, rel=1e-5) ==
                value(model.fs.unit.properties_byproduct[0].flow_vol))
        assert (pytest.approx(998.99, rel=1e-5) ==
...
class TestSettlingPondZO_w_default_removal:
    ...
    def test_solution(self, model):
        ...
        assert (pytest.approx(0.011008174, rel=1e-5) ==
                value(model.fs.unit.properties_treated[0].flow_vol))
        assert (pytest.approx(0.83338, rel=1e-5) ==
                value(model.fs.unit.properties_treated[0].conc_mass_comp["tss"]))
        assert (pytest.approx(90.842, rel=1e-5) ==
                value(model.fs.unit.properties_treated[0].conc_mass_comp["foo"]))
        assert (pytest.approx(9.9183e-4, rel=1e-5) ==
                value(model.fs.unit.properties_byproduct[0].flow_vol))
        assert (pytest.approx(998.99, rel=1e-5) ==
                value(model.fs.unit.properties_byproduct[0].conc_mass_comp["tss"]))
        assert (pytest.approx(8.0659e-07, rel=1e-5) ==
...
def test_costing():
    ...
    m.fs.unit1 = SettlingPondZO(default={
        "property_package": m.fs.params,
        "database": m.db})
    ...
    assert isinstance(m.fs.costing.settling_pond.capital_a_parameter,
                      Var)
    assert isinstance(m.fs.costing.settling_pond.capital_b_parameter,
                      Var)
