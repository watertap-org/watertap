...
#################################################################################
"""
Tests for zero-order media filtration model
"""
...
class TestMediaFiltrationZO:
    ...
    def model(cls):
        ...
        m.fs.unit = MediaFiltrationZO(property_package=m.fs.params, database=m.db)
    ...
    def test_solution(self, model):
        ...
        assert pytest.approx(0.011299, rel=1e-5) == value(
            model.fs.unit.properties_treated[0].flow_vol
        )
        assert pytest.approx(70.8027, rel=1e-5) == value(
            model.fs.unit.properties_treated[0].conc_mass_comp["nonvolatile_toc"]
        )
        assert pytest.approx(44.2517, rel=1e-5) == value(
            model.fs.unit.properties_treated[0].conc_mass_comp["tss"]
        )
        assert pytest.approx(7.01e-4, rel=1e-5) == value(
            model.fs.unit.properties_byproduct[0].flow_vol
        )
        assert pytest.approx(285.307, rel=1e-5) == value(
            model.fs.unit.properties_byproduct[0].conc_mass_comp["nonvolatile_toc"]
        )
        assert pytest.approx(713.267, rel=1e-5) == value(
            model.fs.unit.properties_byproduct[0].conc_mass_comp["tss"]
        )
        assert pytest.approx(6.48e-3, abs=1e-5) == value(model.fs.unit.electricity[0])
...
class TestMediaFiltrationZO_w_default_removal:
    ...
    def test_solution(self, model):
        ...
        assert pytest.approx(0.012299, rel=1e-5) == value(
            model.fs.unit.properties_treated[0].flow_vol
        )
        assert pytest.approx(65.0459, rel=1e-5) == value(
            model.fs.unit.properties_treated[0].conc_mass_comp["nonvolatile_toc"]
        )
        assert pytest.approx(40.6537, rel=1e-5) == value(
            model.fs.unit.properties_treated[0].conc_mass_comp["tss"]
        )
        assert pytest.approx(81.3074, rel=1e-5) == value(
        ...
        assert pytest.approx(7.02e-3, abs=1e-5) == value(model.fs.unit.electricity[0])
...
def test_costing():
    ...
    assert pytest.approx(value(m.fs.costing.LCOW), rel=1e-3) == 0.0106886
    assert pytest.approx(value(m.fs.costing.SEC), rel=1e-3) == 0.00015
