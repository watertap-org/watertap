...
def test_costing():
    ...
    m.fs.unit1 = BufferTankZO(default={
        "property_package": m.fs.params,
        "database": m.db})
    ...
    assert isinstance(m.fs.costing.buffer_tank.capital_a_parameter,
                      Var)
    assert isinstance(m.fs.costing.buffer_tank.capital_b_parameter,
                      Var)
