...
class TestBufferTankZO:
    ...
    def model(self):
        ...
        m.fs.unit = BufferTankZO(property_package=m.fs.params, database=m.db)
...
def test_costing():
    ...
    m.fs.unit1 = BufferTankZO(property_package=m.fs.params, database=m.db)
    ...
    assert isinstance(m.fs.costing.buffer_tank, Block)
    assert isinstance(m.fs.costing.buffer_tank.capital_a_parameter, Var)
    assert isinstance(m.fs.costing.buffer_tank.capital_b_parameter, Var)
    assert isinstance(m.fs.costing.buffer_tank.reference_state, Var)
