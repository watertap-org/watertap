...
class TestDMBRZO:
    ...
    def test_solution(self, model):
        assert (pytest.approx(0.024, rel=1e-5) ==
                value(model.fs.unit.properties_in[0].flow_vol))
        assert (pytest.approx(208.3333, rel=1e-5) ==
        ...
        assert (pytest.approx(8e-10, abs=1e-5) ==
...
def test_costing():
    ...
    assert isinstance(m.fs.costing.dmbr.water_flux,
                      Var)
    assert isinstance(m.fs.costing.dmbr.reactor_cost,
                      Var)
