...
class TestPumpZOdefault:
    ...
    def model(self):
        ...
        m.fs.unit = PumpZO(default={"property_package": m.fs.params, "database": m.db})
    ...
    def test_solution(self, model):
        ...
        assert pytest.approx(1.0060 * 0.051 * 3600, rel=1e-5) == value(
...
class TestPumpZOsubtype:
    ...
    def model(self):
        ...
        m.fs.unit = PumpZO(default={"property_package": m.fs.params, "database": db})
...
def test_costing(subtype):
    ...
    assert isinstance(m.fs.costing.pump.capital_a_parameter, Var)
    assert isinstance(m.fs.costing.pump.capital_b_parameter, Var)
