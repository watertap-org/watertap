...
class Electrodialysis1DData(UnitModelBlockData):
    ...
    **Valid values:** {
    ...
    def build(self):
        ...
        self.dilute_side = ControlVolume1DBlock(
            default={
        ...
        self.dilute_side.add_state_blocks(has_phase_equilibrium=False)
        ...
        self.difference_elements = Set(
            ordered=True,
            initialize=(
                x for x in self.dilute_side.length_domain if x != self.first_element
            ),
        )
        self.nfe = Param(
        ...
        self.concentrate_side = ControlVolume1DBlock(
            default={
                "dynamic": self.config.dynamic,
                "has_holdup": self.config.has_holdup,
                "property_package": self.config.property_package,
        ...
        self.concentrate_side.add_state_blocks(has_phase_equilibrium=False)
        ...
        if not hasattr(self.config, "energy_balance_type"):
            ...
            def eq_isothermal_dilute(self, t, x):
                return (
                    self.dilute_side.properties[t, self.first_element].temperature
        ...
        if not hasattr(self.config, "energy_balance_type"):
            ...
            def eq_isothermal_concentrate(self, t, x):
                return (
                    self.concentrate_side.properties[t, self.first_element].temperature
        ...
        def eq_mass_transfer_term_dilute(self, t, x, p, j):
            ...
            return (
                self.dilute_side.mass_transfer_term[t, x, p, j]
                == -self.nonelec_flux[t, x, p, j] * width
        ...
        def eq_mass_transfer_term_concentrate(self, t, x, p, j):
            width = pyunits.convert(self.cell_width, to_units=units_meta("length"))
            return (
                self.concentrate_side.mass_transfer_term[t, x, p, j]
                == self.nonelec_flux[t, x, p, j] * width
                + self.elec_flux[t, x, p, j] * width
            )
    ...
    def propogate_initial_state(self):
        ...
        for set in self.dilute_side.properties:
            ...
            if "pressure" in self.dilute_side.properties[set].define_state_vars():
                self.dilute_side.properties[set].pressure = value(
                    self.dilute_side.properties[intial_set].pressure
                )
                self.concentrate_side.properties[set].pressure = value(
                    self.concentrate_side.properties[intial_set].pressure
                )

            if "temperature" in self.dilute_side.properties[set].define_state_vars():
                self.dilute_side.properties[set].temperature = value(
                    self.dilute_side.properties[intial_set].temperature
                )
                self.concentrate_side.properties[set].temperature = value(
                    self.concentrate_side.properties[intial_set].temperature
                )
            ...
            if (
                ...
                for ind in self.dilute_side.properties[set].flow_mol_phase_comp:
                    self.dilute_side.properties[set].flow_mol_phase_comp[ind] = value(
                        self.dilute_side.properties[intial_set].flow_mol_phase_comp[ind]
                    )
                ...
                if self.dilute_side.properties[set].is_property_constructed(
                    ...
                    for ind in self.dilute_side.properties[set].flow_mass_phase_comp:
                        self.dilute_side.properties[set].flow_mass_phase_comp[
                            ind
                        ] = value(
                            self.dilute_side.properties[intial_set].flow_mol_phase_comp[
                                ind
                            ]
                        )
    ...
    def calculate_scaling_factors(self):
        ...
        if (iscale.get_scaling_factor(self.dilute_side.length) is None) and (
            ...
            sf = iscale.get_scaling_factor(
                self.dilute_side.length, default=1, warning=False
            )
            ...
            sf = iscale.get_scaling_factor(
                self.concentrate_side.length, default=1, warning=False
            )
        else:
            sf = iscale.get_scaling_factor(
                self.dilute_side.length, default=1, warning=False
        ...
        if (
            iscale.get_scaling_factor(
                self.dilute_side.properties[0, self.first_element].temperature
            )
            is None
        ):
            sf = iscale.get_scaling_factor(
        ...
        if (
            iscale.get_scaling_factor(
                self.concentrate_side.properties[0, self.first_element].temperature
            )
            is None
        ):
            sf = iscale.get_scaling_factor(
                ...
                for t in self.flowsheet().config.time:
                    ...
                    for x in self.difference_elements:
                        ...
                        sf_flow = iscale.get_scaling_factor(
                            self.dilute_side._flow_terms[t, x, p, j]
                        )
                        if sf_flow >= 1000:
                            sf_flow = 1000
                        if sf_flow <= 0.001:
                        ...
                        iscale.set_scaling_factor(
                            self.dilute_side._flow_terms[t, x, p, j], sf_flow
                        )
                        if ix == 0:
                            iscale.set_scaling_factor(
                                self.dilute_side._flow_terms[t, 0, p, j], sf_flow
                            )

                        sf_flow = iscale.get_scaling_factor(
                            self.concentrate_side._flow_terms[t, x, p, j]
                        )
                        if sf_flow >= 1000:
                            sf_flow = 1000
                        if sf_flow <= 0.001:
                            sf_flow = 0.001
                        iscale.set_scaling_factor(
                            self.concentrate_side._flow_terms[t, x, p, j], sf_flow
                        )
                        if ix == 0:
                            iscale.set_scaling_factor(
                                self.concentrate_side._flow_terms[t, 0, p, j], sf_flow
                            )
                        ...
                        if p == "Liq":
                            if j == "H2O":
                                Posm_D = self.dilute_side.properties[
                                    t, x
                                ].pressure_osm_phase[p]
                                Posm_C = self.concentrate_side.properties[
                                    t, x
                                ].pressure_osm_phase[p]
                                L_cem = pyunits.convert(
                                    self.water_permeability_membrane["cem"],
                        ...
                        if abs(value(sf3)) < 0.001:
                            sf3 = 0.001
                        iscale.constraint_scaling_transform(
                            self.eq_mass_transfer_term_dilute[t, x, p, j],
                            1 / abs(value(sf3)),
                        )
                        iscale.constraint_scaling_transform(
        ...
        for t in self.flowsheet().config.time:
            ...
            for x in self.difference_elements:
                ...
                if self.dilute_side.properties[t, x].is_property_constructed(
                    ...
                    for set in self.dilute_side.properties[t, x].true_to_appr_species:
                        ...
                        j = set[1]
                        iscale.constraint_scaling_transform(
                            self.dilute_side.properties[t, x].true_to_appr_species[
                                p, j
                            ],
                            1,
                        )
                        if ix == 0:
                            iscale.constraint_scaling_transform(
                                self.dilute_side.properties[t, 0].true_to_appr_species[
                                    p, j
                                ],
                                1,
                            )
                if self.concentrate_side.properties[t, x].is_property_constructed(
                    "true_to_appr_species"
                ):
                    for set in self.concentrate_side.properties[
                        t, x
                    ].true_to_appr_species:
                        p = set[0]
                        j = set[1]
                        iscale.constraint_scaling_transform(
                            self.concentrate_side.properties[t, x].true_to_appr_species[
                                p, j
                            ],
                            1,
                        )
                ...
                if self.dilute_side.properties[t, x].is_property_constructed(
                    ...
                    for set in self.dilute_side.properties[
                        ...
                        j = set[1]
                        iscale.constraint_scaling_transform(
                            self.dilute_side.properties[t, x].appr_mole_frac_constraint[
                                p, j
                            ],
                            1,
                        )
                ...
                if self.dilute_side.properties[t, x].is_property_constructed(
                    ...
                    for set in self.dilute_side.properties[
                        ...
                        j = set[1]
                        sf = iscale.get_scaling_factor(
                            self.dilute_side.properties[t, x].flow_mol_phase_comp[p, j]
                        )
                        iscale.set_scaling_factor(
                            self.dilute_side.properties[
                                t, x
                ...
                if self.concentrate_side.properties[t, x].is_property_constructed(
                    ...
                    for set in self.concentrate_side.properties[
                        ...
                        j = set[1]
                        sf = iscale.get_scaling_factor(
                            self.concentrate_side.properties[t, x].flow_mol_phase_comp[
                                p, j
                            ]
                        )
                        iscale.set_scaling_factor(
                ...
                if self.dilute_side.properties[t, x].is_property_constructed(
                    ...
                    for set in self.dilute_side.properties[
                        ...
                        j = set[1]
                        sf = iscale.get_scaling_factor(
                            self.dilute_side.properties[t, x].mole_frac_phase_comp[p, j]
                        )
                        iscale.set_scaling_factor(
                            self.dilute_side.properties[
                                t, x
                ...
                if self.concentrate_side.properties[t, x].is_property_constructed(
                    ...
                    for set in self.concentrate_side.properties[
                        ...
                        j = set[1]
                        sf = iscale.get_scaling_factor(
                            self.concentrate_side.properties[t, x].mole_frac_phase_comp[
                                p, j
                            ]
                        )
                        iscale.set_scaling_factor(
    ...
    def _get_stream_table_contents(self, time_point=0):
        return create_stream_table_dataframe(
            {
                "Dilute Side Inlet": self.inlet_dilute,
