...
class ReverseOsmosis1DData(_ReverseOsmosisBaseData):
    ...
    **Valid values:** {
    ...
    def _add_expressions(self):
        ...
        if self.config.has_full_reporting is False:
            ...
            if hasattr(self, "N_Re"):
                ...
                def N_Re_avg(b, t):
                    return sum(b.N_Re[t, x] for x in self.feed_side.length_domain) / self.nfe
            ...
            if hasattr(self, "Kf"):
                ...
                def Kf_avg(b, t, j):
                    return (
                        sum(b.Kf[t, x, j] for x in self.feed_side.length_domain if x > 0) / self.nfe
                    )
    ...
    def _make_performance(self):
        ...
        if (self.config.mass_transfer_coefficient == MassTransferCoefficient.calculated) or (
            self.config.pressure_change_type == PressureChangeType.calculated
        ...
        def eq_recovery_vol_phase(b, t):
            return (
                ...
                / b.feed_side.properties[t, self.feed_side.length_domain.first()].flow_vol_phase[
                    "Liq"
                ]
            )
        ...
        def eq_recovery_mass_phase_comp(b, t, j):
            return (
                ...
                * b.feed_side.properties[t, b.feed_side.length_domain.first()].flow_mass_phase_comp[
                    "Liq", j
                ]
        ...
        def eq_permeate_production(b, t, p, j):
            return b.mixed_permeate[t].get_material_flow_terms(p, j) == sum(
                ...
                for x in b.feed_side.length_domain
                if x != 0
            )
        ...
        def eq_concentration_polarization(b, t, x, j):
            if x == self.feed_side.length_domain.first():
                ...
                if (
                    ...
                    return interface.conc_mass_phase_comp["Liq", j] == bulk.conc_mass_phase_comp[
                        "Liq", j
                    ] * exp(jw / self.Kf[t, x, j]) - js / jw * (exp(jw / self.Kf[t, x, j]) - 1)
        ...
        if self.config.mass_transfer_coefficient == MassTransferCoefficient.calculated:
            ...
            @self.Constraint(
                self.flowsheet().config.time, self.feed_side.length_domain, doc="Sherwood number"
            )
            def eq_N_Sh(b, t, x):
                return b.N_Sh[t, x] == 0.46 * (b.N_Re[t, x] * b.N_Sc[t, x]) ** 0.36

            @self.Constraint(
                self.flowsheet().config.time, self.feed_side.length_domain, doc="Schmidt number"
            )
            def eq_N_Sc(b, t, x):
                bulk = b.feed_side.properties[t, x]
                return (
                    b.N_Sc[t, x] * bulk.dens_mass_phase["Liq"] * bulk.diffus_phase["Liq"]
        ...
        if self.config.mass_transfer_coefficient == MassTransferCoefficient.calculated or (
            ...
            @self.Constraint(
                self.flowsheet().config.time, self.feed_side.length_domain, doc="Reynolds number"
            )
            def eq_N_Re(b, t, x):
                bulk = b.feed_side.properties[t, x]
                return (
        ...
        if (
            ...
            def eq_pressure_drop(b, t):
                return b.deltaP_stage[t] == sum(
                    b.deltaP[t, x] * b.length / b.nfe for x in b.feed_side.length_domain if x != 0
                )
        ...
        def eq_permeate_isothermal(b, t, x):
            return b.feed_side.properties[t, x].temperature == b.permeate_side[t, x].temperature

        # ==========================================================================
        # isothermal conditions at permeate outlet

        @self.Constraint(self.flowsheet().config.time, doc="Isothermal assumption for permeate out")
        ...
        def eq_equal_temp_interface(b, t, x):
            if x == self.feed_side.length_domain.first():
                ...
                bulk = b.properties[t, x]
                interface = b.properties_interface[t, x]
            return interface.temperature == bulk.temperature

        # PRESSURE
        @self.feed_side.Constraint(
            self.flowsheet().config.time, self.feed_side.length_domain, doc="Pressure at interface"
        )
        def eq_equal_pressure_interface(b, t, x):
            if x == self.feed_side.length_domain.first():
                ...
                bulk = b.properties[t, x]
                interface = b.properties_interface[t, x]
            return interface.pressure == bulk.pressure
        ...
        def eq_equal_flow_vol_interface(b, t, x):
            if x == self.feed_side.length_domain.first():
                ...
                bulk = b.properties[t, x]
                interface = b.properties_interface[t, x]
            return interface.flow_vol_phase["Liq"] == bulk.flow_vol_phase["Liq"]
    ...
    def initialize(
        ...
        flags_feed_side = blk.feed_side.initialize(
            outlvl=outlvl, optarg=optarg, solver=solver, state_args=feed_side_args
        )
        init_log.info("Feed-side initialization complete. Initialize permeate-side. ")
        flags_permeate_side = blk.permeate_side.initialize(
            outlvl=outlvl, optarg=optarg, solver=solver, state_args=permeate_side_args
        )
        ...
        flags_mixed_permeate = blk.mixed_permeate.initialize(
            outlvl=outlvl, optarg=optarg, solver=solver, state_args=permeate_block_args
        )
        ...
        # Step 2: Solve unit
        init_log.info(
            "Initialization Step 1 complete: all state blocks initialized."
        ...
        check_solve(
            ...
            logger=init_log,
            fail_flag=False,
            checkpoint="Initialization Step 2: solve indexed blocks",
        )
    ...
    def _get_performance_contents(self, time_point=0):
        ...
        for j in self.config.property_package.solute_set:
            if (
                ...
                or self.config.has_full_reporting
            ):
                var_dict[
            ...
            if (
                ...
                var_dict[f"{j} Concentration @Inlet,Bulk"] = feed_inlet.conc_mass_phase_comp[
                    "Liq", j
                ]
            if (
                ...
                var_dict[f"{j} Concentration @Outlet,Bulk"] = feed_outlet.conc_mass_phase_comp[
                    "Liq", j
                ]
            if (
                ...
                var_dict[f"{j} Permeate Concentration"] = permeate.conc_mass_phase_comp["Liq", j]
        if (
            interface_outlet.is_property_constructed("pressure_osm")
            or self.config.has_full_reporting
        ):
            var_dict["Osmotic Pressure @Outlet,Membrane-Interface "] = interface_outlet.pressure_osm
        ...
        if (
            ...
            var_dict["Osmotic Pressure @Inlet,Membrane-Interface"] = interface_inlet.pressure_osm
    ...
    def calculate_scaling_factors(self):
        ...
        for j in self.config.property_package.component_list:
            iscale.set_scaling_factor(self.permeate_side[0, 0].flow_mass_phase_comp["Liq", j], 1e5)
                ...
                if x == self.feed_side.length_domain.first():
                    if comp.is_solvent():
                        iscale.set_scaling_factor(
                            v, 5e4
                        )  # inverse of initial value from flux_mass_phase_comp_initialize
                    elif comp.is_solute():
                        iscale.set_scaling_factor(
                            v, 1e6
                        )  # inverse of initial value from flux_mass_phase_comp_initialize
        ...
        for (t, x, p, j), v in self.eq_mass_flux_equal_mass_transfer.items():
            if iscale.get_scaling_factor(v) is None:
                if x == self.feed_side.length_domain.first():
                    ...
                    sf = iscale.get_scaling_factor(
                        self.flux_mass_phase_comp[t, x, p, j]
                    ) * iscale.get_scaling_factor(self.width)
                    iscale.set_scaling_factor(v, sf / 100.0)
        ...
        if hasattr(self, "eq_area_cross"):
            ...
            iscale.constraint_scaling_transform(self.eq_area_cross, sf * 10.0)
        ...
        if hasattr(self, "eq_dP_dx"):
            for ind, c in self.eq_dP_dx.items():
                sf = iscale.get_scaling_factor(self.deltaP[ind]) * iscale.get_scaling_factor(
                    self.dh
                )
                iscale.constraint_scaling_transform(c, sf)
