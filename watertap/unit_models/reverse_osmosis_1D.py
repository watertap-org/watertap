    ...
    **Valid values:** {
    ...
    def _add_expressions(self):
        ...
        if self.config.has_full_reporting is False:
            ...
            if hasattr(self, "N_Re"):
                ...
                def N_Re_avg(b, t):
                    return (
                        sum(b.N_Re[t, x] for x in self.feed_side.length_domain)
                        / self.nfe
                    )
            ...
            if hasattr(self, "Kf"):
                ...
                def Kf_avg(b, t, j):
                    return (
                        sum(
                            b.Kf[t, x, j] for x in self.feed_side.length_domain if x > 0
                        )
                        / self.nfe
                    )
        ...
        def eq_recovery_vol_phase(b, t):
            return (
                ...
                / b.feed_side.properties[
                    t, self.feed_side.length_domain.first()
                ].flow_vol_phase["Liq"]
            )
        ...
        def eq_concentration_polarization(b, t, x, j):
            if x == self.feed_side.length_domain.first():
                ...
                if (
                    ...
                    jw = (
                        self.flux_mass_phase_comp[t, x, "Liq", "H2O"]
                        / self.dens_solvent
                    )
    ...
    def initialize(
        ...
        source = blk.feed_side.properties[
            blk.flowsheet().config.time.first(), blk.feed_side.length_domain.first()
        ]
