...
def test_state_block_basic_attributes(model):
    ...
    for p in model.fs.state[0].phase_list:
        assert model.fs.state[0].get_enthalpy_flow_terms(p) is model.fs.state[0]._enth_flow_term
        assert model.fs.state[0].get_energy_density_terms(p) is model.fs.state[0]._enth_dens_term
        ...
        for j in model.fs.state[0].component_list:
            ...
            if j == "H2O":
                assert (
                    ...
                    is model.fs.water_props.dens_mass
                )
            else:
                assert (
                    model.fs.state[0].get_material_density_terms(p, j)
    ...
    assert model.fs.state[0].default_material_balance_type() is MaterialBalanceType.componentTotal

    assert model.fs.state[0].default_energy_balance_type() is EnergyBalanceType.enthalpyTotal
...
def test_state_block_scaling(model):
    ...
    assert model.fs.state[0].scaling_factor[model.fs.state[0].conc_mass_comp["A"]] == 100
    assert model.fs.state[0].scaling_factor[model.fs.state[0].conc_mass_comp["B"]] == 5e-2
    assert model.fs.state[0].scaling_factor[model.fs.state[0].conc_mass_comp["C"]] == 100
    assert model.fs.state[0].scaling_factor[model.fs.state[0].temperature] == 1e-2
    assert model.fs.state[0].scaling_factor[model.fs.state[0].pressure] == 1e-5

    assert model.fs.state[0].scaling_factor[model.fs.state[0].flow_mass_comp["H2O"]] == 1
    assert model.fs.state[0].scaling_factor[model.fs.state[0].flow_mass_comp["A"]] == 1e5
    assert model.fs.state[0].scaling_factor[model.fs.state[0].flow_mass_comp["B"]] == 5e1
    assert model.fs.state[0].scaling_factor[model.fs.state[0].flow_mass_comp["C"]] == 1e5

    assert model.fs.state[0].scaling_factor[model.fs.state[0]._enth_dens_term] == 1e-5
    assert model.fs.state[0].scaling_factor[model.fs.state[0]._enth_flow_term] == 1e-4
...
def test_CV_integration(model):
    model.fs.cv = ControlVolume0DBlock(default={"property_package": model.fs.water_props})
