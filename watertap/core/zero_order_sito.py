...
class SITOBaseData(UnitModelBlockData):
    ...
    def build(self):
        ...
        if not hasattr(self, "_has_deltaP_treated"):
            raise NotImplementedError(
                f"{self.name} derived class has not been implemented " f"_has_deltaP_treated."
            )
        if not hasattr(self, "_has_deltaP_byproduct"):
            raise NotImplementedError(
                f"{self.name} derived class has not been implemented " f"_has_deltaP_byproduct."
            )
        ...
        self.properties_treated = self.config.property_package.build_state_block(
            self.flowsheet().time, doc="Material properties of treated water", default=tmp_dict_2
        )
        self.properties_byproduct = self.config.property_package.build_state_block(
            self.flowsheet().time, doc="Material properties of byproduct stream", default=tmp_dict_2
        )
        ...
        def water_recovery_equation(b, t):
            return (
                b.recovery_vol[t] * b.properties_in[t].flow_vol == b.properties_treated[t].flow_vol
            )
        ...
        def flow_balance(b, t):
            return (
                ...
                == b.properties_treated[t].flow_vol + b.properties_byproduct[t].flow_vol
            )
        ...
        def solute_removal_equation(b, t, j):
            return (
                b.removal_mass_solute[t, j] * b.properties_in[t].conc_mass_comp[j]
        ...
        def solute_treated_equation(b, t, j):
            return (1 - b.removal_mass_solute[t, j]) * b.properties_in[t].conc_mass_comp[
                j
            ] == b.recovery_vol[t] * b.properties_treated[t].conc_mass_comp[j]

        # Pressure drop
        @self.Constraint(self.flowsheet().time, doc="Treated stream pressure equation")
        def treated_pressure_constraint(b, t):
            if self._has_deltaP_treated:
                dp = b.deltaP_treated[t]
            else:
                dp = 0
            return b.properties_in[t].pressure + dp == b.properties_treated[t].pressure

        @self.Constraint(self.flowsheet().time, doc="Byproduct stream pressure equation")
        def byproduct_pressure_constraint(b, t):
            if self._has_deltaP_byproduct:
                dp = b.deltaP_byproduct[t]
            else:
                dp = 0
            return b.properties_in[t].pressure + dp == b.properties_byproduct[t].pressure

        # Temperature equality
        @self.Constraint(self.flowsheet().time, doc="Treated stream temperature equality")
        def treated_temperature_equality(b, t):
            return b.properties_in[t].temperature == b.properties_treated[t].temperature

        @self.Constraint(self.flowsheet().time, doc="Byproduct stream temperature equality")
        def byproduct_temperature_equality(b, t):
            return b.properties_in[t].temperature == b.properties_byproduct[t].temperature

    def initialize(blk, state_args=None, outlvl=idaeslog.NOTSET, solver=None, optarg=None):
        """
        ...
        if state_args is None:
            state_args = {}
            state_dict = blk.properties_in[blk.flowsheet().time.first()].define_port_members()
        ...
        blk.properties_treated.initialize(
            outlvl=outlvl, optarg=optarg, solver=solver, state_args=state_args, hold_state=False
        )
    ...
    def calculate_scaling_factors(self):
        ...
        for t, v in self.flow_balance.items():
            iscale.constraint_scaling_transform(
                v,
                iscale.get_scaling_factor(self.properties_in[t].flow_vol, default=1, warning=False),
        ...
        for t, v in self.byproduct_pressure_constraint.items():
            iscale.constraint_scaling_transform(
                v,
                iscale.get_scaling_factor(
                    self.properties_in[t].pressure, default=1e-5, warning=False
                ),
            )  # would just be a duplicate of above
        ...
        for t, v in self.byproduct_temperature_equality.items():
            iscale.constraint_scaling_transform(
                v,
                iscale.get_scaling_factor(
                    self.properties_in[t].temperature, default=1e-2, warning=False
                ),
            )  # would just be a duplicate of above
