...
def print_constraints_close_to_bounds(m, rel_tol=1e-4, abs_tol=1e-5):
    ...
    """
    for con in m.component_data_objects(ctype=Constraint, descend_into=True, active=True):
        if con.equality:
            continue
...
def _eval_close(obj, val, rel_tol, abs_tol):
    lb = value(obj.lb)
    ub = value(obj.ub)
    if lb is not None and ub is not None and isclose(lb, ub, rel_tol=rel_tol, abs_tol=abs_tol):
        return
