...
class SeawaterParameterData(PhysicalParameterBlock):
    ...
    def build(self):
        ...
        self.dens_mass_param_A1 = Var(
            within=Reals, initialize=9.999e2, units=dens_units, doc="Mass density parameter A1"
        )
        ...
        self.dens_mass_param_B1 = Var(
            within=Reals, initialize=8.020e2, units=dens_units, doc="Mass density parameter B1"
        )
        ...
        diffus_param_dict = {"0": 1.51e-9, "1": -2.00e-9, "2": 3.01e-8, "3": -1.22e-7, "4": 1.53e-7}
...
class _SeawaterStateBlock(StateBlock):
    ...
    def initialize(
        ...
        init_log.info("Property initialization: {}.".format(idaeslog.condition(results)))
    ...
    def calculate_state(
        self, var_args=None, hold_state=False, outlvl=idaeslog.NOTSET, solver=None, optarg=None
    ):
        """
                ...
                if var[ind].is_fixed():
                    ...
                    if value(var[ind]) != val:
                        raise ConfigurationError(
                            ...
                            "".format(
                                sb_name=sb.name, v_name=var.name, val=val, val_2=value(var[ind])
                            )
                        )
                else:
...
class SeawaterStateBlockData(StateBlockData):
    ...
    def _mole_frac_phase_comp(self):
        ...
        def rule_mole_frac_phase_comp(b, j):
            return b.mole_frac_phase_comp["Liq", j] == b.flow_mol_phase_comp["Liq", j] / sum(
    ...
    def _osm_coeff(self):
        self.osm_coeff = Var(
            initialize=1, bounds=(1e-8, 10), units=pyunits.dimensionless, doc="Osmotic coefficient"
        )
    ...
    def _pressure_sat(self):
        ...
        def rule_pressure_sat(b):  # vapor pressure, eq. 5 and 6 in Nayar et al.(2016)
            ...
            return b.pressure_sat == psatw * exp(
                b.params.pressure_sat_param_B1 * s + b.params.pressure_sat_param_B2 * s ** 2
            )
    ...
    def _therm_cond_phase(self):
        ...
        def rule_therm_cond_phase(b):  # thermal conductivity, eq. 13 in Sharqawy  et al. (2010)
            ...
            s = b.mass_frac_phase_comp["Liq", "TDS"] * 1000 * pyunits.g / pyunits.kg
            log10_ksw = log10(
                b.params.therm_cond_phase_param_1 + b.params.therm_cond_phase_param_2 * s
                ...
                1 - t / (b.params.therm_cond_phase_param_7 + b.params.therm_cond_phase_param_8 * s)
            ) ** (
                1 / 3
            )
            return (
        ...
        if self.is_property_constructed("pressure_osm"):
            if iscale.get_scaling_factor(self.pressure_osm) is None:
                iscale.set_scaling_factor(
                    self.pressure_osm, iscale.get_scaling_factor(self.pressure)
                )
