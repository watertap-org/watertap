...
class PropertyTestHarness:
        ...
        for v in m.fs.properties.component_data_objects(Var):
            if not v.is_fixed():
                raise PropertyRuntimeError(
                    ...
                    "property parameter block must be fixed by default".format(v_name=v.name)
                )
    ...
    def test_stateblock_statistics(self, frame_stateblock):
        ...
        if number_variables(blk) != stats["number_variables"]:
            raise PropertyValueError(
                ...
                "expected ".format(num=number_variables(blk), num_test=stats["number_variables"])
            )
        if number_total_constraints(blk) != stats["number_total_constraints"]:
            raise PropertyValueError(
                "The number of constraints were {num}, but {num_test} was "
                "expected ".format(
                    num=number_total_constraints(blk), num_test=stats["number_total_constraints"]
                )
            )
        if number_unused_variables(blk) != stats["number_unused_variables"]:
            raise PropertyValueError(
                ...
                "expected ".format(
                    num=number_unused_variables(blk), num_test=stats["number_unused_variables"]
                )
            )
        if degrees_of_freedom(blk) != stats["default_degrees_of_freedom"]:
            raise PropertyValueError(
                ...
                "expected ".format(
                    num=degrees_of_freedom(blk), num_test=stats["default_degrees_of_freedom"]
                )
            )
    ...
    def test_scaling(self, frame_stateblock):
        ...
        if len(unscaled_var_list) != 0:
            ...
            raise PropertyAttributeError(
                "The following variable(s) are unscaled: {lst}".format(lst=unscaled_var_name_list)
            )
    ...
    def check_constraint_status(blk, tol=1e-8):
        ...
        for c in blk.component_data_objects(ctype=Constraint, active=True, descend_into=True):
