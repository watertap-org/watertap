...
if __name__ == '__main__':  
    ...
    # plt.xticks(x)
    plt.ylabel('LCOW \$/m$^3$')
    plt.title('Contribution to LCOW')
    plt.legend()
    plt.show() 
    ...
    fig3, ax = plt.subplots(figsize=(5, 3), layout='constrained')
    lineslco = ax.plot(x, oper1, label="Operetional cost")
    lineslco = ax.plot(x, anual1, label="Capital cost")
    lineslco = ax.plot(x, lco, label="LCOW")
    ax.set_xlabel('Plant flow [m$^3$/day]')
    ax.set_ylabel('Costs \$/m$^3$')
    ax.set_title('LCOW')
    ax.legend()
    plt.show()  
