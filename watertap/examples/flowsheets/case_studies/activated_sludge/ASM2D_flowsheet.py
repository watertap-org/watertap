...
def build_flowsheet():
    ...
    def scale_variables(m):
        for var in m.fs.component_data_objects(pyo.Var, descend_into=True):
            ...
            if "reaction_rate[R1]" in var.name:
                iscale.set_scaling_factor(var, 1e11)
            if "reaction_rate[R2]" in var.name:
                iscale.set_scaling_factor(var, 1e6)
            if "reaction_rate[R3]" in var.name:
                iscale.set_scaling_factor(var, 1e6)
            if "reaction_rate[R4]" in var.name:
                iscale.set_scaling_factor(var, 1e9)
            if "reaction_rate[R5]" in var.name:
                iscale.set_scaling_factor(var, 1e11)
            if "reaction_rate[R6]" in var.name:
                iscale.set_scaling_factor(var, 1e5)
            if "reaction_rate[R7]" in var.name:
                iscale.set_scaling_factor(var, 1e5)
            if "reaction_rate[R8]" in var.name:
                iscale.set_scaling_factor(var, 1e5)
            if "reaction_rate[R9]" in var.name:
                iscale.set_scaling_factor(var, 1e5)
            if "reaction_rate[R10]" in var.name:
                iscale.set_scaling_factor(var, 1e8)
            if "reaction_rate[R11]" in var.name:
                iscale.set_scaling_factor(var, 1e11)
            if "reaction_rate[R12]" in var.name:
                iscale.set_scaling_factor(var, 1e8)
            if "reaction_rate[R13]" in var.name:
                iscale.set_scaling_factor(var, 1e11)
            if "reaction_rate[R14]" in var.name:
                iscale.set_scaling_factor(var, 1e8)
            if "reaction_rate[R15]" in var.name:
                iscale.set_scaling_factor(var, 1e9)
            if "reaction_rate[R16]" in var.name:
                iscale.set_scaling_factor(var, 1e8)
            if "reaction_rate[R17]" in var.name:
                iscale.set_scaling_factor(var, 1e9)
            if "reaction_rate[R18]" in var.name:
                iscale.set_scaling_factor(var, 1e6)
            if "reaction_rate[R19]" in var.name:
                iscale.set_scaling_factor(var, 1e11)
            if "reaction_rate[R20]" in var.name:
                iscale.set_scaling_factor(var, 1e11)
            if "reaction_rate[R21]" in var.name:
                iscale.set_scaling_factor(var, 1e13)

            if "rate_reaction_extent[0.0,R1]" in var.name:
                iscale.set_scaling_factor(var, 1e5)
            if "rate_reaction_extent[0.0,R2]" in var.name:
                iscale.set_scaling_factor(var, 1e4)
            if "rate_reaction_extent[0.0,R3]" in var.name:
                iscale.set_scaling_factor(var, 1e4)
            if "rate_reaction_extent[0.0,R4]" in var.name:
                iscale.set_scaling_factor(var, 1e4)
            if "rate_reaction_extent[0.0,R5]" in var.name:
                iscale.set_scaling_factor(var, 1e6)
            if "rate_reaction_extent[0.0,R6]" in var.name:
                iscale.set_scaling_factor(var, 1e4)
            if "rate_reaction_extent[0.0,R7]" in var.name:
                iscale.set_scaling_factor(var, 1e4)
            if "rate_reaction_extent[0.0,R8]" in var.name:
                iscale.set_scaling_factor(var, 1e4)
            if "rate_reaction_extent[0.0,R9]" in var.name:
                iscale.set_scaling_factor(var, 1e3)
            if "rate_reaction_extent[0.0,R10]" in var.name:
                iscale.set_scaling_factor(var, 1e5)
            if "rate_reaction_extent[0.0,R11]" in var.name:
                iscale.set_scaling_factor(var, 1e5)
            if "rate_reaction_extent[0.0,R12]" in var.name:
                iscale.set_scaling_factor(var, 1e4)
            if "rate_reaction_extent[0.0,R13]" in var.name:
                iscale.set_scaling_factor(var, 1e4)
            if "rate_reaction_extent[0.0,R14]" in var.name:
                iscale.set_scaling_factor(var, 1e5)
            if "rate_reaction_extent[0.0,R15]" in var.name:
                iscale.set_scaling_factor(var, 1e6)
            if "rate_reaction_extent[0.0,R16]" in var.name:
                iscale.set_scaling_factor(var, 1e5)
            if "rate_reaction_extent[0.0,R17]" in var.name:
                iscale.set_scaling_factor(var, 1e5)
            if "rate_reaction_extent[0.0,R18]" in var.name:
                iscale.set_scaling_factor(var, 1e5)
            if "rate_reaction_extent[0.0,R19]" in var.name:
                iscale.set_scaling_factor(var, 1e4)
            if "rate_reaction_extent[0.0,R20]" in var.name:
                iscale.set_scaling_factor(var, 1e11)
            if "rate_reaction_extent[0.0,R21]" in var.name:
                iscale.set_scaling_factor(var, 1e11)

            if "rate_reaction_generation[0.0,Liq,S_O2]" in var.name:
                iscale.set_scaling_factor(var, 1e3)
            if "rate_reaction_generation[0.0,Liq,S_F]" in var.name:
                iscale.set_scaling_factor(var, 1e4)
            if "rate_reaction_generation[0.0,Liq,S_A]" in var.name:
                iscale.set_scaling_factor(var, 1e4)
            if "rate_reaction_generation[0.0,Liq,S_NH4]" in var.name:
                iscale.set_scaling_factor(var, 1e5)
            if "rate_reaction_generation[0.0,Liq,S_NO3]" in var.name:
                iscale.set_scaling_factor(var, 1e4)
            if "rate_reaction_generation[0.0,Liq,S_PO4]" in var.name:
                iscale.set_scaling_factor(var, 1e5)
            if "rate_reaction_generation[0.0,Liq,S_I]" in var.name:
                iscale.set_scaling_factor(var, 1e2)
            if "rate_reaction_generation[0.0,Liq,S_N2]" in var.name:
                iscale.set_scaling_factor(var, 1e4)
            if "rate_reaction_generation[0.0,Liq,X_I]" in var.name:
                iscale.set_scaling_factor(var, 1e5)
            if "rate_reaction_generation[0.0,Liq,X_S]" in var.name:
                iscale.set_scaling_factor(var, 1e4)
            if "rate_reaction_generation[0.0,Liq,X_H]" in var.name:
                iscale.set_scaling_factor(var, 1e4)
            if "rate_reaction_generation[0.0,Liq,X_PAO]" in var.name:
                iscale.set_scaling_factor(var, 1e5)
            if "rate_reaction_generation[0.0,Liq,X_PP]" in var.name:
                iscale.set_scaling_factor(var, 1e5)
            if "rate_reaction_generation[0.0,Liq,X_PHA]" in var.name:
                iscale.set_scaling_factor(var, 1e5)
            if "rate_reaction_generation[0.0,Liq,X_AUT]" in var.name:
                iscale.set_scaling_factor(var, 1e5)
            if "rate_reaction_generation[0.0,Liq,X_MeOH]" in var.name:
                iscale.set_scaling_factor(var, 1e10)
            if "rate_reaction_generation[0.0,Liq,X_MeP]" in var.name:
                iscale.set_scaling_factor(var, 1e10)
            if "rate_reaction_generation[0.0,Liq,X_TSS]" in var.name:
                iscale.set_scaling_factor(var, 1e4)
