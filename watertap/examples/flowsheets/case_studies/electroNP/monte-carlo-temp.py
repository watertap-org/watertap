...
def model():
    ...
    T = [298.15, 300.15, 304.15, 306.15, 308.15]

    for j in T:

        m.fs.AD.inlet.temperature.fix(j)
        m.fs.AD.liquid_outlet.temperature.fix(j)
        print(j)

        for i in range(N):

            noise_x = np.random.normal(loc=0, scale=st_dev_x)
            m.fs.AD.inlet.conc_mass_comp[0, "X_PP"].fix(x_s + noise_x)
            print(i)
            print(value(m.fs.AD.inlet.conc_mass_comp[0, "X_PP"]))
            ...
            try:
                ...
                if (status.solver.status == SolverStatus.ok) and (
                    ...
                    Feed.append(value(m.fs.AD.inlet.conc_mass_comp[0, "X_PP"]))
                    ...
                    S_NH4.append(
                        value(m.fs.electroNP.treated.conc_mass_comp[0, "S_NH4"])
                    )
                    S_NO3.append(
                        value(m.fs.electroNP.treated.conc_mass_comp[0, "S_NO3"])
                    )
                    S_O2.append(value(m.fs.electroNP.treated.conc_mass_comp[0, "S_O2"]))
                    S_PO4.append(
                        value(m.fs.electroNP.treated.conc_mass_comp[0, "S_PO4"])
                    )
                    X_AUT.append(
                        value(m.fs.electroNP.treated.conc_mass_comp[0, "X_AUT"])
                    )
                    X_H.append(value(m.fs.electroNP.treated.conc_mass_comp[0, "X_H"]))
                    X_I.append(value(m.fs.electroNP.treated.conc_mass_comp[0, "X_I"]))
                    X_PAO.append(
                        value(m.fs.electroNP.treated.conc_mass_comp[0, "X_PAO"])
                    )
                    X_PHA.append(
                        value(m.fs.electroNP.treated.conc_mass_comp[0, "X_PHA"])
                    )
                    ...
                    lcow.append(
                        value(
                            units.convert(
                                m.fs.costing.LCOW,
                                to_units=units.USD_2018 / units.m**3,
                            )
                        )
                    )
                    e_cost.append(
