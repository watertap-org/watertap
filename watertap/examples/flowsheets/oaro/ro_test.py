...
def build(water_recovery=None):
    ...
    # m.fs.unit.recovery_mass_phase_comp[0, "Liq", "H2O"].fix(0.4)
...
def display_design(m):
    ...
    print(
        "OARO Stage water recovery: %.3f"
        % (value(m.fs.unit.recovery_mass_phase_comp[0, "Liq", "H2O"]),)
    )
    print(
...
def display_state(m):
    ...
    def print_state(s, b):
        ...
        print(
            s.ljust(20)
            + ": %.2f kg/h,  %.0f, %.3f g/L, %.1f bar, %.4f kg/s, %.4f kg/s"
            % (
                flow_mass,
