...
def build():
    ...
    m.fs.separator = Separator(
        default={"property_package": m.fs.properties, "outlet_list": ["inlet_diluate", "inlet_concentrate"]}
    ...
    m.fs.costing.add_specific_energy_consumption(m.fs.product.properties[0].flow_vol_phase['Liq'])

    # Add Arcs
    m.fs.s01 = Arc(source=m.fs.feed.outlet, destination=m.fs.separator.inlet)
    m.fs.s02 = Arc(source=m.fs.separator.inlet_diluate, destination=m.fs.EDstack.inlet_diluate)
    m.fs.s03 = Arc(source=m.fs.separator.inlet_concentrate, destination=m.fs.EDstack.inlet_concentrate)
...
def optimize_system(m, solver=None):
    ...
    m.fs.eq_product_quality = Constraint(
        expr=m.fs.product.properties[0].conc_mass_phase_comp["Liq", "Na_+"]
        <= 0.393
    )
...
def display_model_metrics(m):
    ...
    fp={"Feed": [value(m.fs.feed.properties[0].flow_vol_phase['Liq']), value(sum(
            m.fs.feed.properties[0].conc_mass_phase_comp["Liq", j] for j in m.fs.properties.ion_set 
        ))], "Product": [value(m.fs.product.properties[0].flow_vol_phase['Liq']), value(sum(
            m.fs.product.properties[0].conc_mass_phase_comp["Liq", j] for j in m.fs.properties.ion_set 
        ))], "Disposal":[value(m.fs.disposal.properties[0].flow_vol_phase['Liq']), value(sum(
            m.fs.disposal.properties[0].conc_mass_phase_comp["Liq", j] for j in m.fs.properties.ion_set 
        ))] }
    fp_table = DataFrame(data = fp, index = ['Volume Flow Rate (m3/s)', 'Total Ion Mass Concentration (kg/m3)'])
    print (fp_table)

    print("---Performance Metrics---")

    pm_table = DataFrame( data = [value(m.fs.EDstack.water_recovery_mass[0]),
    value(m.fs.EDstack.cell_width * m.fs.EDstack.cell_length * m.fs.EDstack.cell_pair_num),
     value(m.fs.EDstack.voltage_applied[0]),
     value(m.fs.costing.specific_energy_consumption),
     value(m.fs.costing.LCOW)], columns=['value'], index = ['Water recovery by mass', 'Total membrane area (aem or cem), m2',
      'Operation Voltage, V',
