...
def build(
    ...
    #.fs.costing.erd_pressure_exchanger_cost.fix(535)

    m.fs.NumberOfStacks = Param(initialize=number_of_stacks)
    m.fs.Stack = RangeSet(m.fs.NumberOfStacks)
    m.fs.Stack_D = RangeSet(m.fs.NumberOfStacks) # "D" stands for the diluate channel. 
    m.fs.Stack_C = RangeSet(m.fs.NumberOfStacks)
    m.fs.ED_Stacks = RangeSet(1, m.fs.NumberOfStacks)
    ...
    # Add the pumps
    m.fs.Pump_D = Pump(m.fs.Stack_D, default={"property_package": m.fs.properties})
    m.fs.Pump_C = Pump(m.fs.Stack_C, default={"property_package": m.fs.properties})
    for pump in m.fs.Pump_D.values():
    ...
    for pump in m.fs.Pump_C.values():
        pump.costing = UnitModelCostingBlock(
            default={
                "flowsheet_costing_block": m.fs.costing,
                "costing_method": cost_high_pressure_pump_lsrro,
