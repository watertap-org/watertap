...
def main():
    ...
    initialize_system(m, outlvl=idaeslog.INFO_HIGH)
    ...
    print("First solve - simulation minimizing Q_ext")
    solver = get_solver()
    ...
    set_up_optimization(
        m
    )  # unfixes decision variables and sets objective to minimize LCOW
    results = solve(m, tee=False)
    print(results.solver.termination_condition)
    if results.solver.termination_condition == "infeasible":
        debug_infeasible(m.fs, solver)
...
def build():
    ...
    # m.fs.hx_U_constraint = Constraint(expr=m.fs.hx_distillate.overall_heat_transfer_coefficient == m.fs.hx_brine.overall_heat_transfer_coefficient)
...
def add_Q_ext(m, time_point=None):
    ...
    m.fs.Q_ext = Var(
        time_point, initialize=0, units=pyunits.J / pyunits.s
    )  # , bounds=(0,1e7))
...
def add_costing(m):
    ...
    m.fs.costing.annual_operating_costs = Expression(
        expr=m.fs.costing.total_capital_cost * m.fs.costing.factor_capital_annualization
...
def initialize_system(m, solver=None, outlvl=idaeslog.INFO):
    if solver is None:
        solver = get_solver()
    ...
    m.fs.pump_feed.initialize_build(optarg=optarg, outlvl=outlvl)
    ...
    m.fs.separator_feed.initialize_build(optarg=optarg, outlvl=outlvl)
    ...
    m.fs.hx_distillate.initialize_build(outlvl=outlvl)
    ...
    m.fs.hx_brine.initialize_build(outlvl=outlvl)

    # initialize mixer
    propagate_state(m.fs.s05)
    propagate_state(m.fs.s06)
    m.fs.mixer_feed.initialize_build(outlvl=outlvl)
    m.fs.mixer_feed.pressure_equality_constraints[0, 2].deactivate()
    ...
    m.fs.evaporator.initialize_build(
        delta_temperature_in=50, delta_temperature_out=10, outlvl=outlvl
    )  # fixes and unfixes those values
    m.fs.Q_ext[0].unfix()

    # initialize compressor
    propagate_state(m.fs.s08)
    m.fs.compressor.initialize_build(outlvl=outlvl)

    # initialize condenser
    propagate_state(m.fs.s09)
    m.fs.condenser.initialize_build(
        heat=-m.fs.evaporator.heat_transfer.value, outlvl=outlvl
    )

    # initialize brine pump
    propagate_state(m.fs.s10)
    m.fs.pump_brine.initialize_build(optarg=optarg, outlvl=outlvl)
    ...
    m.fs.pump_distillate.initialize_build(optarg=optarg, outlvl=outlvl)

    m.fs.costing.initialize(outlvl=outlvl)
...
def display_results(m):
    ...
    print(
        "Electricity cost:                        ",
        m.fs.costing.electricity_base_cost.value,
    )
    print(
        "Total investment factor:                 ",
        m.fs.costing.factor_total_investment.value,
    )
    print("LCOW:                                    ", m.fs.costing.LCOW.value)
    print(
        "Capex-Opex ratio                         ",
        value(m.fs.costing.LCOW_percentage["capex_opex_ratio"]),
    )
...
def save_results(m, filename=None):
    ...
    feed_concentration = (
        round(m.fs.feed.properties[0].mass_frac_phase_comp["Liq", "TDS"].value, 3)
        * 1000
    )
    ...
    if filename is None:
        filename = (
            "C:/Users/carso/Documents/MVC/watertap_results/wf_"
            + str(feed_concentration)
            + "_rr_"
            + str(recovery)
            + ".csv"
        )
    ...
    outputs["Feed mass flow water"] = [
        m.fs.feed.properties[0].flow_mass_phase_comp["Liq", "H2O"].value
    ]
    outputs["Feed mass flow salt"] = [
        m.fs.feed.properties[0].flow_mass_phase_comp["Liq", "TDS"].value
    ]
    outputs["Feed mass fraction"] = [
        m.fs.feed.properties[0].mass_frac_phase_comp["Liq", "TDS"].value
    ]
    ...
    outputs["Brine mass flow water"] = [
        m.fs.brine.properties[0].flow_mass_phase_comp["Liq", "H2O"].value
    ]
    outputs["Brine mass flow salt"] = [
        m.fs.brine.properties[0].flow_mass_phase_comp["Liq", "TDS"].value
    ]
    outputs["Brine temperature"] = [
        m.fs.evaporator.properties_brine[0].temperature.value
    ]
    outputs["Brine pressure"] = [m.fs.evaporator.properties_brine[0].pressure.value]

    # Vapor
    outputs["Vapor mass flow"] = [
        m.fs.evaporator.properties_vapor[0].flow_mass_phase_comp["Vap", "H2O"].value
    ]
    outputs["Vapor temperature"] = [
        m.fs.evaporator.properties_vapor[0].temperature.value
    ]
    outputs["Vapor pressure"] = [m.fs.evaporator.properties_vapor[0].pressure.value]

    # Compressed vapor
    outputs["Compressed vapor temperature"] = [
        m.fs.compressor.control_volume.properties_out[0].temperature.value
    ]
    outputs["Compressed vapor pressure"] = [
        m.fs.compressor.control_volume.properties_out[0].pressure.value
    ]

    # Condensed vapor/distillate
    outputs["Distillate temperature"] = [
        m.fs.condenser.control_volume.properties_out[0].temperature.value
    ]
    outputs["Distillate pressure"] = [
        m.fs.condenser.control_volume.properties_out[0].pressure.value
    ]

    # Exiting distillate
    outputs["Exiting distillate pressure"] = [
    ...
    outputs["Evaporator approach temperature in"] = [
        m.fs.evaporator.delta_temperature_in.value
    ]
    outputs["Evaporator approach temperature out"] = [
        m.fs.evaporator.delta_temperature_out.value
    ]
    ...
    outputs["Preheater split ratio"] = [
        m.fs.separator_feed.split_fraction[0, "hx_distillate_cold"].value
    ]

    # Feed exiting distillate heat exchanger
    outputs["Feed exiting distillate hx temperature"] = [
        m.fs.hx_distillate.cold.properties_out[0].temperature.value
    ]
    outputs["Feed exiting distillate hx pressure"] = [
        m.fs.hx_distillate.cold.properties_out[0].pressure.value
    ]

    # Feed exiting brine heat exchanger
    outputs["Feed exiting brine hx temperature"] = [
        m.fs.hx_brine.cold.properties_out[0].temperature.value
    ]
    outputs["Feed exiting brine hx pressure"] = [
        m.fs.hx_brine.cold.properties_out[0].pressure.value
    ]

    # Preheated feed
    outputs["Preheated feed temperature"] = [
        m.fs.evaporator.properties_feed[0].temperature.value
    ]
    outputs["Preheated feed pressure"] = [
        m.fs.evaporator.properties_feed[0].pressure.value
    ]

    # Distillate heat exchanger performance
    outputs["Distillate hx area"] = [m.fs.hx_distillate.area.value]
    outputs["Distillate hx delta temp in"] = [
        m.fs.hx_distillate.delta_temperature_in[0].value
    ]
    outputs["Distillate hx delta temp out"] = [
        m.fs.hx_distillate.delta_temperature_out[0].value
    ]
    outputs["Distillate hx heat transfer"] = [m.fs.hx_distillate.heat_duty[0].value]
    outputs["Distillate hx overall heat transfer coefficient"] = [
        m.fs.hx_distillate.overall_heat_transfer_coefficient[0].value
    ]
    ...
    outputs["Brine hx overall heat transfer coefficient"] = [
        m.fs.hx_brine.overall_heat_transfer_coefficient[0].value
    ]
    ...
    outputs["Distillate pump captial cost"] = [
        m.fs.pump_distillate.costing.capital_cost.value
    ]
    outputs["Brine pump captial cost"] = [
        m.fs.pump_distillate.costing.capital_cost.value
    ]
    outputs["Distillate hx capital cost"] = [
        m.fs.hx_distillate.costing.capital_cost.value
    ]
    ...
    outputs["Aggregate electricity flow cost"] = [
        value(m.fs.costing.aggregate_flow_costs["electricity"])
    ]
    outputs["Total investment cost"] = [m.fs.costing.total_capital_cost.value]
    outputs["Total investment factor"] = [m.fs.costing.factor_total_investment.value]
    outputs["Evaporator material factor"] = [
        m.fs.costing.evaporator_material_factor_cost.value
    ]
    outputs["Distillate hx material factor"] = [
        m.fs.costing.heat_exchanger_material_factor_cost.value
    ]
    outputs["Brine hx material factor"] = [
        m.fs.costing.heat_exchanger_material_factor_cost.value
    ]
    outputs["Electricity cost"] = [m.fs.costing.electricity_base_cost.value]
    outputs["Capital annualization factor"] = [
        m.fs.costing.factor_capital_annualization.value
    ]
    outputs["Utilization factor"] = [m.fs.costing.utilization_factor.value]
    outputs["MLC cost"] = [
        value(m.fs.costing.maintenance_labor_chemical_operating_cost)
    ]
    ...
    outputs["CC normalized feed pump"] = [
        value(m.fs.costing.MVC_captial_cost_percentage["feed_pump"])
    ]
    outputs["CC normalized distillate pump"] = [
        value(m.fs.costing.MVC_captial_cost_percentage["distillate_pump"])
    ]
    outputs["CC normalized brine pump"] = [
        value(m.fs.costing.MVC_captial_cost_percentage["brine_pump"])
    ]
    outputs["CC normalized distiallte hx"] = [
        value(m.fs.costing.MVC_captial_cost_percentage["hx_distillate"])
    ]
    outputs["CC normalized brine hx"] = [
        value(m.fs.costing.MVC_captial_cost_percentage["hx_brine"])
    ]
    outputs["CC normalized mixer"] = [
        value(m.fs.costing.MVC_captial_cost_percentage["mixer"])
    ]
    outputs["CC normalized evaportor"] = [
        value(m.fs.costing.MVC_captial_cost_percentage["evaporator"])
    ]
    outputs["CC normalized compressor"] = [
        value(m.fs.costing.MVC_captial_cost_percentage["compressor"])
    ]

    # Normalized LCOW costs
    outputs["LCOW normalized feed pump"] = [
        value(m.fs.costing.LCOW_percentage["feed_pump"])
    ]
    outputs["LCOW normalized distillate pump"] = [
        value(m.fs.costing.LCOW_percentage["distillate_pump"])
    ]
    outputs["LCOW normalized brine pump"] = [
        value(m.fs.costing.LCOW_percentage["brine_pump"])
    ]
    outputs["LCOW normalized distillate hx"] = [
        value(m.fs.costing.LCOW_percentage["hx_distillate"])
    ]
    outputs["LCOW normalized brine hx"] = [
        value(m.fs.costing.LCOW_percentage["hx_brine"])
    ]
    outputs["LCOW normalized mixer"] = [value(m.fs.costing.LCOW_percentage["mixer"])]
    outputs["LCOW normalized evaporator"] = [
        value(m.fs.costing.LCOW_percentage["evaporator"])
    ]
    outputs["LCOW normalized compressor"] = [
        value(m.fs.costing.LCOW_percentage["compressor"])
    ]
    outputs["LCOW normalized electricity"] = [
        value(m.fs.costing.LCOW_percentage["electricity"])
    ]
    outputs["LCOW normalized MLC"] = [value(m.fs.costing.LCOW_percentage["MLC"])]
    outputs["LCOW normalized capex"] = [
        value(m.fs.costing.LCOW_percentage["capital_costs"])
    ]
    outputs["LCOW normalized opex"] = [
        value(m.fs.costing.LCOW_percentage["operating_costs"])
    ]
