...
class GACData(InitializationMixin, UnitModelBlockData):
    ...
    **EnergyBalanceType.energyPhase** - energy balances for each phase.}""",
    ...
    def build(self):
        ...
        self.min_st_surrogate = SurrogateBlock(concrete=True)
        self.throughput_surrogate = SurrogateBlock(concrete=True)
        self.ele_throughput_surrogate_1 = SurrogateBlock(concrete=True)
        self.ele_throughput_surrogate_2 = SurrogateBlock(concrete=True)
        self.ele_throughput_surrogate_3 = SurrogateBlock(concrete=True)
        self.ele_throughput_surrogate_4 = SurrogateBlock(concrete=True)
        self.ele_throughput_surrogate_5 = SurrogateBlock(concrete=True)
        ...
        self.min_st_surrogate.build_model(
            min_st_surrogate,
            input_vars=[self.freund_ninv, self.N_Bi],
            output_vars=[self.min_N_St],
        )
        self.throughput_surrogate.build_model(
            throughput_surrogate,
            input_vars=[self.freund_ninv, self.N_Bi, self.conc_ratio_replace],
            output_vars=[self.throughput],
        )
        self.ele_throughput_surrogate_1.build_model(
            throughput_surrogate,
            input_vars=[
                self.freund_ninv,
                self.N_Bi,
                self.ele_conc_ratio_replace[1],
            ],
            output_vars=[self.ele_throughput[1]],
        )
        self.ele_throughput_surrogate_2.build_model(
            throughput_surrogate,
            input_vars=[
                self.freund_ninv,
                self.N_Bi,
                self.ele_conc_ratio_replace[2],
            ],
            output_vars=[self.ele_throughput[2]],
        )
        self.ele_throughput_surrogate_3.build_model(
            throughput_surrogate,
            input_vars=[
                self.freund_ninv,
                self.N_Bi,
                self.ele_conc_ratio_replace[3],
            ],
            output_vars=[self.ele_throughput[3]],
        )
        self.ele_throughput_surrogate_4.build_model(
            throughput_surrogate,
            input_vars=[
                self.freund_ninv,
                self.N_Bi,
                self.ele_conc_ratio_replace[4],
            ],
            output_vars=[self.ele_throughput[4]],
        )
        self.ele_throughput_surrogate_5.build_model(
            throughput_surrogate,
            input_vars=[
                self.freund_ninv,
                self.N_Bi,
                self.ele_conc_ratio_replace[5],
            ],
            output_vars=[self.ele_throughput[5]],
        )
    ...
    def initialize_build(
        ...
        if not check_optimal_termination(res):
            ...
            self.display()
            badly_scaled_var_list = badly_scaled_var_generator(self)
