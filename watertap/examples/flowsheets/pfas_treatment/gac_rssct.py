...
class GACData(InitializationMixin, UnitModelBlockData):
    ...
    **EnergyBalanceType.energyPhase** - energy balances for each phase.}""",
    ...
    def build(self):
        ...
        def rule_a_parameters(b):
            ...
            freund_ninv_param_closest = freund_ninv_param_list[
                min(
                    ...
                    key=lambda i: abs(freund_ninv_param_list[i] - b.freund_ninv.value),
                )
            ]
            if b.N_Bi.value < 10:
        ...
        self.eq_a_parameters = Piecewise(rule=rule_a_parameters)

        def rule_b_parameters(b):
            ...
            N_Bi_param_closest = N_Bi_param_list[
                min(
                    ...
                    key=lambda i: abs(N_Bi_param_list[i] - b.N_Bi.value),
                )
            ]
            lookup = throughput_param_df.loc[
                freund_ninv_param_closest, N_Bi_param_closest
            ]

            return (
                b.b0
                == throughput_param_df.loc[
                    freund_ninv_param_closest, N_Bi_param_closest
                ]
            )

        self.eq_b_parameters = Constraint(rule=rule_b_parameters)
