...
class TestIXDemo:
    ...
    def test_build_model(self, ix_0D):
        ...
        # test configrations
        assert len(m.fs.ion_exchange.config) == 12
        ...
        assert m.fs.ion_exchange.config.diffusion_control == DiffusionControlType.liquid
    ...
    def test_specific_operating_conditions(self, ix_0D):
        ...
        assert value(m.fs.costing.LCOW) == pytest.approx(0.2207154, abs=1e-3)
    ...
    def test_optimization(self, ix_0D):
        ...
        assert value(m.fs.ion_exchange.bed_depth) == pytest.approx(1.622805, abs=1e-4)
        assert value(m.fs.ion_exchange.t_breakthru) == pytest.approx(
            133415.81129, abs=1e-4
        )
        ...
        assert value(m.fs.costing.LCOW) == pytest.approx(0.1428129, abs=0.001)
