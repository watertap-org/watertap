...
def build_components(m, pretrt_type="NF", **kwargs):
    kwargs_desalination = {
        k: kwargs[k]
        for k in ("has_desal_feed", "is_twostage", "has_ERD", "RO_type", "RO_base", "RO_level",)
    }
    ...
    m.fs.system_recovery = Expression(expr=product_water_sb.flow_vol / feed_flow_vol)
    ...
    m.fs.RO_recovery = Var(
        initialize=0.5, bounds=(0.01, 0.99), doc="Total volumetric water recovery for RO"
    )
...
def build(m, **kwargs):
    ...
    translator_block.build_tb(
        m, base_inlet="ion", base_outlet=kwargs["RO_base"], name_str="tb_pretrt_to_desal"
    )
...
def set_optimization_components(m, system_recovery, **kwargs):
    ...
    if kwargs["RO_type"] == "0D":
        m.fs.RO.flux_mass_io_phase_comp[0, "out", "Liq", "H2O"].setlb(
