...
def build_components(m, pretrt_type="NF", **kwargs):
    ...
    gypsum_saturation_index.build(
        m, section="desalination", pretrt_type=pretrt_type, **kwargs
    )
    ...
    m.fs.annual_water_production = Expression(
        expr=pyunits.convert(
            product_water_sb.flow_vol, to_units=pyunits.m ** 3 / pyunits.year
        )
...
def build(m, **kwargs):
    ...
    m.fs.s_pretrt_tb = Arc(
        source=m.fs.feed.outlet, destination=m.fs.tb_pretrt_to_desal.inlet
    )
