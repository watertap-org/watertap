...
def build_ZONF(m, base="ion"):
    ...
    if base not in ["ion"]:
        raise ValueError("Unexpected property base {base} for build_ZONF" "".format(base=base))
    prop = property_models.get_prop(m, base=base)

    m.fs.NF = NanofiltrationZO(default={"property_package": prop, "has_pressure_change": False})
    ...
    m.fs.NF.eq_electroneutrality = Constraint(
        expr=0
        == sum(
            charge_comp[j] * m.fs.NF.feed_side.properties_out[0].conc_mol_phase_comp["Liq", j]
