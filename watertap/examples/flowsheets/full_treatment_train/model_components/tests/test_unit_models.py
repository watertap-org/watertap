...
def test_unit_separator_SepNF_ion():
    ...
    assert value(
        m.fs.NF.mixed_state[0].mass_frac_phase_comp["Liq", "Na"]
    ) == pytest.approx(11122e-6, rel=1e-3)
    assert value(
        m.fs.NF.mixed_state[0].mass_frac_phase_comp["Liq", "Cl"]
    ) == pytest.approx(20317e-6, rel=1e-3)
    assert value(m.fs.NF.split_fraction[0, "permeate", "Cl"]) == pytest.approx(
        0.7753, rel=1e-3
    )
    assert value(
    ...
    assert value(
        m.fs.NF.permeate.flow_mass_phase_comp[0, "Liq", "Na"]
    ) == pytest.approx(1.000e-2, rel=1e-3)
    assert value(
        m.fs.NF.permeate.flow_mass_phase_comp[0, "Liq", "Cl"]
    ) == pytest.approx(1.575e-2, rel=1e-3)
    assert value(
        m.fs.NF.retentate.flow_mass_phase_comp[0, "Liq", "Ca"]
    ) == pytest.approx(3.438e-4, rel=1e-3)
    assert value(
        m.fs.NF.retentate.flow_mass_phase_comp[0, "Liq", "Mg"]
    ) == pytest.approx(1.255e-3, rel=1e-3)
    assert value(
        m.fs.NF.retentate.flow_mass_phase_comp[0, "Liq", "SO4"]
    ) == pytest.approx(1.922e-3, rel=1e-3)
...
def test_unit_0DRO_simple():
    ...
    assert value(
        m.fs.RO.permeate.flow_mass_phase_comp[0, "Liq", "H2O"]
    ) == pytest.approx(0.3389, rel=1e-3)
    assert value(
        m.fs.RO.permeate.flow_mass_phase_comp[0, "Liq", "TDS"]
    ) == pytest.approx(7.875e-5, rel=1e-3)
    assert value(
        m.fs.RO.retentate.flow_mass_phase_comp[0, "Liq", "H2O"]
    ) == pytest.approx(0.6261, rel=1e-3)
    assert value(
        m.fs.RO.retentate.flow_mass_phase_comp[0, "Liq", "TDS"]
...
def test_unit_ZONF():
    ...
    assert value(
        m.fs.NF.permeate.flow_mass_phase_comp[0, "Liq", "H2O"]
    ) == pytest.approx(0.8350, rel=1e-3)
    assert value(
        m.fs.NF.permeate.flow_mass_phase_comp[0, "Liq", "Ca"]
    ) == pytest.approx(6.897e-5, rel=1e-3)
    assert value(
        m.fs.NF.permeate.flow_mass_phase_comp[0, "Liq", "Cl"]
    ) == pytest.approx(1.475e-2, rel=1e-3)
