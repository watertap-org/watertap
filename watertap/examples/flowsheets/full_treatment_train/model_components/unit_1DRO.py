...
def build_RO(m, base="TDS", level="simple", name_str="RO"):
    ...
    """
    if base not in ["TDS"]:
        raise ValueError("Unexpected property base {base} for build_RO" "".format(base=base))
    prop = property_models.get_prop(m, base=base)

    if level == "simple":
        raise ValueError("Unexpected RO level {level} for build_RO" "".format(level=level))
        ...
        raise ValueError("Unexpected argument {level} for level in build_RO" "".format(level=level))
