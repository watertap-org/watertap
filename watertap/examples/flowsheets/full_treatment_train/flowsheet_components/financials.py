...
def add_costing_param_block(self):
    ...
    b.hp_pump_cost = Var(
        initialize=53 / 1e5 * 3600, doc="High pressure pump cost [$/W]"
    )
...
def get_system_costing(self):
    ...
    b.LCOW = Var(
        initialize=1, domain=NonNegativeReals, doc="Levelized cost of water [$/m3]"
    )
...
def _make_vars(self, section=None):
    ...
    self.capital_cost = Var(
        initialize=1e5, domain=NonNegativeReals, doc="Unit capital cost [$]"
    )
    ...
    if section not in ["pretreatment", "primary", "post_treatment"]:
        ...
        setattr(
            self,
            section,
            Expression(expr=self.capital_cost * crf + self.operating_cost),
        )
...
def Mixer_costing(self, mixer_type="default", section=None, cost_capacity=False):
    ...
    if mixer_type == "default":
        ...
        if cost_capacity:
            self.eq_capital_cost = Constraint(
                expr=self.capital_cost
                == 16972 * self.lime_lbs_per_day ** 0.5435 * self.cost_esc
            )
        elif not cost_capacity:
...
def pressure_changer_costing(
    self, pump_type="centrifugal", section=None, cost_capacity=False
):
    _make_vars(self, section)
        ...
        if cost_capacity:
            ...
            self.eq_capital_cost = Constraint(
                ...
                == (
                    self.a
                    + self.b
                    * (b_PC.control_volume.properties_in[0].flow_vol * 1000) ** self.n
                )
                * self.Fm
                * self.cost_esc
                / (pyunits.m ** 3 / pyunits.s)
