...
def add_costing_param_block(self):
    ...
    b.load_factor = Var(initialize=0.9, doc="Load factor [fraction of uptime]")
    b.factor_total_investment = Var(
        initialize=2, doc="Total investment factor [investment cost/equipment cost]"
    )
    b.factor_labor_maintenance = Var(
        initialize=0.02, doc="Labor & maintenance factor [fraction of investment cost/year]"
    )
    b.factor_capital_annualization = Var(
        initialize=0.1, doc="Capital annualization factor [fraction of investment cost/year]"
    )
    b.factor_membrane_replacement = Var(
        initialize=0.2, doc="Membrane replacement factor [fraction of membrane replaced/year]"
    )
    b.electricity_cost = Var(initialize=0.07, doc="Electricity cost [$/kWh]")
    b.RO_mem_cost = Var(initialize=30, doc="Membrane cost [$/m2]")
    ...
    b.hp_pump_cost = Var(initialize=53 / 1e5 * 3600, doc="High pressure pump cost [$/W]")
    b.pxr_cost = Var(initialize=535, doc="Pressure exchanger cost [$/(m3/h)]")
...
def get_system_costing(self):
    ...
    b.capital_cost_total = Var(
        initialize=1e3, domain=NonNegativeReals, doc="Total capital cost [$]"
    )
    b.investment_cost_total = Var(
        initialize=1e3, domain=NonNegativeReals, doc="Total investment cost [$]"
    )
    ...
    b.operating_cost_total = Var(
        initialize=1e3, domain=NonNegativeReals, doc="Total operating cost [$/year]"
    )
    b.LCOW = Var(initialize=1, domain=NonNegativeReals, doc="Levelized cost of water [$/m3]")
    ...
    for b_unit in self.component_objects(Block, descend_into=True):
        if hasattr(b_unit, "costing"):
            ...
            if hasattr(b_unit.costing, "eq_operating_cost") and hasattr(
                b_unit.costing.eq_operating_cost, "body"
            ):
    ...
    b.electricity_cost_total = Expression(expr=sum(electricity_cost_var_lst))
    b.pretreatment_cost_total = Expression(expr=sum(pretreatment_cost_var_lst))
    b.primary_cost_total = Expression(expr=sum(primary_cost_var_lst))
    b.post_treatment_cost_total = Expression(expr=sum(post_treatment_cost_var_lst))
    b.eq_LCOW = Constraint(
        expr=b.LCOW
        == (
            b.investment_cost_total * self.costing_param.factor_capital_annualization
            + b.operating_cost_total
        )
        / (self.annual_water_production / (pyunits.m ** 3 / pyunits.year))
    )


def _make_vars(self, section=None):
    # build generic costing variables (all costing models need these vars)
    self.capital_cost = Var(initialize=1e5, domain=NonNegativeReals, doc="Unit capital cost [$]")
    self.operating_cost = Var(
        initialize=1e5, domain=Reals, bounds=(0, 1e6), doc="Unit operating cost [$/year]"
    )
...
def ReverseOsmosis_costing(self, RO_type="standard", section="primary"):
    ...
    if RO_type == "standard":
        ...
        self.eq_operating_cost = Constraint(
            ...
            == b_fs.costing_param.factor_membrane_replacement
            * b_fs.costing_param.RO_mem_cost
            * b_RO.area
            / pyunits.m ** 2
        )
        ...
        self.eq_operating_cost = Constraint(
            expr=self.operating_cost
            == b_fs.costing_param.factor_membrane_replacement
            * b_fs.costing_param.RO_high_pressure_mem_cost
            * b_RO.area
            / pyunits.m ** 2
        )
    else:
        raise NotImplementedError(
            "RO type of {RO_type} is not implemented in the financials file"
...
def Nanofiltration_costing(self, section="pretreatment"):
    ...
    self.eq_operating_cost = Constraint(
        expr=self.operating_cost
        == b_fs.costing_param.factor_membrane_replacement
        * b_fs.costing_param.NF_mem_cost
        * b_NF.area
        / pyunits.m ** 2
    )
...
def Separator_costing(self, section=None, cost_capacity=False):
    ...
    if cost_capacity:
        ...
        self.eq_capital_cost = Constraint(
            expr=self.capital_cost
            == (self.a + self.b * (b_m.outlet_state[0].flow_vol * 1000) ** self.n)
            * self.Fm
            * self.cost_esc
        ...
        self.separator_unit_capex = Param(
            initialize=361,
            mutable=True,
        ...
        self.eq_capital_cost = Constraint(
            expr=self.capital_cost
            == self.separator_unit_capex
            * b_m.outlet_state[0].flow_vol
            * 1000
            * self.cost_esc
            * pyunits.s
            * pyunits.m ** -3
...
def Mixer_costing(self, mixer_type="default", section=None, cost_capacity=False):
    ...
    if mixer_type == "default":
        if cost_capacity:
            ...
            self.eq_capital_cost = Constraint(
                expr=self.capital_cost
                == (self.a + self.b * (b_m.mixed_state[0].flow_vol * 1000) ** self.n)
                * self.Fm
                * self.cost_esc
                / (pyunits.m ** 3 / pyunits.s)
            )
            ...
            self.eq_capital_cost = Constraint(
                expr=self.capital_cost
                == self.mixer_unit_capex
                * b_m.mixed_state[0].flow_vol
                * 1000
                * self.cost_esc
                * pyunits.s
                * pyunits.m ** -3
        ...
        if cost_capacity:
            ...
            self.eq_capital_cost = Constraint(
                expr=self.capital_cost
                == 479.87
                * (b_m.inlet_stream_state[0].flow_vol * 3600 * 24 / (pyunits.m ** 3 / pyunits.s))
                ** 0.604
                * self.cost_esc
            )
        elif not cost_capacity:
            # assume linear cost per daily capacity based on median of digitized data cited above
            self.naocl_unit_capex = Param(
                initialize=5.08,
                mutable=True,
                units=pyunits.day * pyunits.m ** -3,
                doc="Capex per daily plant capacity",
            )
            self.eq_capital_cost = Constraint(
                expr=self.capital_cost
                == self.naocl_unit_capex
        ...
        self.lime_lbs_per_day = Expression(
            expr=2.205
            * 3600
            * 24
            * 74.09e-3
        ...
        if cost_capacity:
            self.eq_capital_cost = Constraint(
                expr=self.capital_cost == 16972 * self.lime_lbs_per_day ** 0.5435 * self.cost_esc
            )
        elif not cost_capacity:
            # assume linear cost per lb feed per day based on minimum of digitized data cited above
            self.caoh2_unit_capex = Param(
                initialize=792.8,
                mutable=True,
                units=pyunits.dimensionless,
                doc="Capex per lb feed per day",
            )
            self.eq_capital_cost = Constraint(
                expr=self.capital_cost
                == self.caoh2_unit_capex
                * self.lime_lbs_per_day
                * self.cost_esc
        ...
        raise NotImplementedError(
            "mixer_type of {mixer_type} is not implemented in the financials file"
...
def pressure_changer_costing(self, pump_type="centrifugal", section=None, cost_capacity=False):
    ...
    if pump_type == "high pressure":
        ...
        self.eq_capital_cost = Constraint(
            expr=self.capital_cost
            == b_fs.costing_param.hp_pump_cost * b_PC.work_mechanical[0] / pyunits.W
        )
        ...
        self.eq_operating_cost = Constraint(
            ...
            * b_fs.costing_param.electricity_cost
            / 3600
            / 1000
        )
        ...
        if cost_capacity:
            ...
            self.eq_capital_cost = Constraint(
                ...
                == (
                    self.a
                    + self.b * (b_PC.control_volume.properties_in[0].flow_vol * 1000) ** self.n
                )
                * self.Fm
                * self.cost_esc
                / (pyunits.m ** 3 / pyunits.s)
            ...
            self.pump_unit_capex = Param(
                initialize=889, mutable=True, units=pyunits.dimensionless, doc="Capex per liter/s"
            )
            self.eq_capital_cost = Constraint(
                expr=self.capital_cost
                == self.pump_unit_capex
                * b_PC.control_volume.properties_in[0].flow_vol
                * 1000
                * self.cost_esc
                / (pyunits.m ** 3 / pyunits.s)
            )
        ...
        raise NotImplementedError(
            "pump type of {pump_type} is not implemented in the financials file"
