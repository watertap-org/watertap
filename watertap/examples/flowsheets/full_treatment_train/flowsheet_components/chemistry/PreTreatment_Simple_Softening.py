...
def build_simple_softening_unit(
    model,
    ...
    model.fs.softening_unit = StoichiometricReactor(
        default={
    ...
    model.fs.softening_unit.inlet.mole_frac_comp[0, "Mg(OH)2"].fix(0.0)
    model.fs.softening_unit.inlet.mole_frac_comp[0, "CaCO3"].fix(0.0)
    model.fs.softening_unit.inlet.mole_frac_comp[0, "Ca(OH)2"].fix(
        0.0
    )  # temperory fix for initializing
...
def display_results_of_softening(softening_unit):
    ...
    total_hardness1 = (
        50000 * 2 * softening_unit.outlet.mole_frac_comp[0, "Ca(HCO3)2"].value * total_molar_density
    )
    total_hardness2 = (
        50000 * 2 * softening_unit.outlet.mole_frac_comp[0, "Mg(HCO3)2"].value * total_molar_density
    )
...
def run_softening_example():
    ...
    state_args, stoich_extents = approximate_chemical_state_args(
        model.fs.softening_unit, model.fs.softening_rxn_params, softening_reaction_config
    )

    calculate_chemical_scaling_factors(
        model.fs.softening_unit,
        model.fs.softening_thermo_params,
        model.fs.softening_rxn_params,
        state_args,
    )
