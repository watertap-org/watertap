...
simple_naocl_reaction_config = {
    ...
    "equilibrium_reactions": {
        ...
        "HOCl_Ka": {
            "stoichiometry": {("Liq", "HOCl"): -1, ("Liq", "H_+"): 1, ("Liq", "OCl_-"): 1},
            "heat_of_reaction": constant_dh_rxn,
            "equilibrium_constant": van_t_hoff,
            "equilibrium_form": log_power_law_equil,
            "concentration_form": ConcentrationForm.moleFraction,
            "parameter_data": {
                ...
                # By default, reaction orders follow stoichiometry
                #    manually set reaction order here to override
                "reaction_order": {("Liq", "HOCl"): -1, ("Liq", "H_+"): 1, ("Liq", "OCl_-"): 1},
            }
            # End parameter_data
        }
        # End R2
...
def build_simple_naocl_chlorination_unit(
    model,
    ...
    model.fs.simple_naocl_unit = EquilibriumReactor(
        default={
            "property_package": model.fs.simple_naocl_thermo_params,
    ...
    model.fs.simple_naocl_unit.control_volume.properties_out[
        0.0
    ].log_mole_frac_phase_comp_true.setlb(-50)
    model.fs.simple_naocl_unit.control_volume.properties_out[
        0.0
    ].log_mole_frac_phase_comp_true.setub(0.001)
    model.fs.simple_naocl_unit.control_volume.properties_out[0.0].mole_frac_phase_comp.setub(1.001)

    model.fs.simple_naocl_unit.inlet.mole_frac_comp[0, "H_+"].fix(0.0)
    model.fs.simple_naocl_unit.inlet.mole_frac_comp[0, "OH_-"].fix(0.0)
    model.fs.simple_naocl_unit.inlet.mole_frac_comp[0, "HOCl"].fix(0.0)
    model.fs.simple_naocl_unit.inlet.mole_frac_comp[0, "Cl_-"].fix(0.0)
    ...
    model.fs.simple_naocl_unit.inlet.mole_frac_comp[0, "OCl_-"].fix(
        total_chlorine_inlet / total_molar_density
    )
    model.fs.simple_naocl_unit.inlet.mole_frac_comp[0, "Na_+"].fix(
        total_chlorine_inlet / total_molar_density
    )
    ...
    dr = (
        model.fs.simple_naocl_unit.inlet.flow_mol[0].value
        * model.fs.simple_naocl_unit.inlet.mole_frac_comp[0, "OCl_-"].value
    )
    dr = dr * 74.44 * 1000
    ...
    def _dosing_rate_cons(blk):
        return (
            blk.dosing_rate
            == blk.inlet.flow_mol[0] * blk.inlet.mole_frac_comp[0, "OCl_-"] * 74.44 * 1000
        )
    ...
    def _free_chlorine_cons(blk):
        return (
            blk.free_chlorine
            == (
                (blk.control_volume.properties_out[0.0].conc_mol_phase_comp["Liq", "HOCl"] / 1000)
                + (
                    blk.control_volume.properties_out[0.0].conc_mol_phase_comp["Liq", "OCl_-"]
                    / 1000
                )
            )
            * 70900
...
def display_results_of_chlorination(chlorination_unit):
    ...
    total_salt = (
        value(chlorination_unit.outlet.mole_frac_comp[0, "Na_+"]) * total_molar_density * 23
    )
    total_salt += (
        value(chlorination_unit.outlet.mole_frac_comp[0, "Cl_-"]) * total_molar_density * 35.4
    )
    ...
    hocl = (
        value(
            chlorination_unit.control_volume.properties_out[0.0].conc_mol_phase_comp["Liq", "HOCl"]
        )
        / 1000
    ) / (chlorination_unit.free_chlorine.value / 70900)
    print("\t % HOCl: \t" + str(hocl * 100))
    ocl = (
        value(
            chlorination_unit.control_volume.properties_out[0.0].conc_mol_phase_comp["Liq", "OCl_-"]
        )
        / 1000
    ) / (chlorination_unit.free_chlorine.value / 70900)
