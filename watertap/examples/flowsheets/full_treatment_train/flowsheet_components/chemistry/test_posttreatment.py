...
def test_ideal_naocl_mixer():
    ...
    assert model.fs.ideal_naocl_mixer_unit.dosing_rate.value == pytest.approx(0.4, rel=1e-3)
    assert model.fs.ideal_naocl_mixer_unit.outlet.flow_mol[0].value == pytest.approx(
        10.00001074690966, rel=1e-3
    )
    assert model.fs.ideal_naocl_mixer_unit.outlet.mole_frac_comp[0, "OCl_-"].value == pytest.approx(
        5.373448801531194e-07, rel=1e-3
    )
...
def test_ideal_naocl_chlorination_full_block():
    ...
    assert model.fs.ideal_naocl_mixer_unit.dosing_rate.value == pytest.approx(
        0.9504457542440085, rel=1e-3
    )
    assert model.fs.ideal_naocl_mixer_unit.outlet.flow_mol[0].value == pytest.approx(
        25.000025535888078, rel=1e-3
    )
    assert model.fs.ideal_naocl_mixer_unit.outlet.mole_frac_comp[0, "OCl_-"].value == pytest.approx(
        5.107172398859054e-07, rel=1e-3
    )

    assert model.fs.ideal_naocl_chlorination_unit.free_chlorine.value == pytest.approx(2, rel=1e-3)
    assert model.fs.ideal_naocl_chlorination_unit.outlet.mole_frac_comp[
        0, "OCl_-"
