...
def test_flowsheet_mvp_NF_bypass_twostage_1():
    ...
    assert value(m.fs.RO2.retentate.flow_mass_phase_comp[0, "Liq", "H2O"]) == pytest.approx(
        0.2408, rel=1e-3
    )
    assert value(m.fs.RO2.retentate.flow_mass_phase_comp[0, "Liq", "TDS"]) == pytest.approx(
        2.605e-2, rel=1e-3
    )
...
def test_flowsheet_mvp_cost_optimization():
    ...
    assert value(m.fs.mixer_permeate.outlet.flow_mass_phase_comp[0, "Liq", "H2O"]) == pytest.approx(
        0.6778, rel=1e-3
    )
