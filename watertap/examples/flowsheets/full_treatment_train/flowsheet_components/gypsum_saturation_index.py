...
def build(m, section="desalination", pretrt_type="NF", **kwargs):
    if section == "desalination":
        ...
        raise ValueError(
            "{section} is not an expected section for building the saturation index"
    ...
    if section == "desalination":
        ...
        m.fs.desal_saturation.cp_modulus = Expression(
            expr=sb_conc_inter.conc_mass_phase_comp["Liq", "TDS"]
            / sb_conc.conc_mass_phase_comp["Liq", "TDS"]
        )
        ...
        m.fs.desal_saturation.eq_pressure = Constraint(expr=sb_eNRTL.pressure == sb_conc.pressure)
        ...
        if pretrt_type == "NF":
            ...
            def eq_flow_mol_balance(b, j):
                if j in ["Cl_-", "Na_+"]:
                    bulk_flow = sb_dilute.flow_mol_phase_comp[
                        "Liq", comp_match_dict[j]
                    ] - sb_perm.flow_mass_phase_comp["Liq", "TDS"] / (
                        58.44e-3 * pyunits.kg / pyunits.mol
                    )
                    return (
                        sb_eNRTL.flow_mol_phase_comp["Liq", j]
        ...
        m.fs.pretrt_saturation.eq_pressure = Constraint(expr=sb_eNRTL.pressure == sb_conc.pressure)
