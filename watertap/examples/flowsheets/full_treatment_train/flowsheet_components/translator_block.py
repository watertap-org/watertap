...
def build_tb(m, base_inlet="ion", base_outlet="TDS", name_str=None):
    ...
    setattr(
        ...
        name_str,
        Translator(
            default={"inlet_property_package": prop_inlet, "outlet_property_package": prop_outlet}
        ),
    )
    ...
    blk.eq_equal_pressure = Constraint(expr=blk.inlet.pressure[0] == blk.outlet.pressure[0])
    constraint_scaling_transform(
