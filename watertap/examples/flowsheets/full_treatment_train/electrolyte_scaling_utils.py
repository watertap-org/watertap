...
def approximate_chemical_state_args(
    ...
    if contains_stoich_reactions == True:
        for rid in rxn_params.rate_reaction_idx:
            ...
            for phase, species in reaction_config["rate_reactions"][rid]["stoichiometry"]:
                ...
                if species in fixed:
                    extent = (
                        unit.inlet.mole_frac_comp[0, species].value
            ...
            for phase, species in reaction_config["rate_reactions"][rid]["stoichiometry"]:
                state_args["mole_frac_comp"][species] = (
                    unit.inlet.mole_frac_comp[0, species].value
                    + extent
                    * reaction_config["rate_reactions"][rid]["stoichiometry"][phase, species]
                )
    ...
    if was_H_changed == False and was_OH_changed == False:
        ...
        state_args["mole_frac_comp"]["H_+"] = (
            10 ** -14 / (state_args["mole_frac_comp"]["OH_-"] * 55.6) / 55.6
        )
    elif was_H_changed == True and was_OH_changed == False:
        state_args["mole_frac_comp"]["OH_-"] = (
            10 ** -14 / (state_args["mole_frac_comp"]["H_+"] * 55.6) / 55.6
        )
    else:
    ...
    try:
        ...
        for i in unit.control_volume.inherent_reaction_extent_index:
            ...
            iscale.constraint_scaling_transform(
                unit.control_volume.properties_out[0.0].inherent_equilibrium_constraint[i[1]], 0.1
            )
    except:
        pass
