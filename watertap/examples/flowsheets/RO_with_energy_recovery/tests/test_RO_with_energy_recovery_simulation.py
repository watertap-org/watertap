...
class TestROwithPX:
    ...
    def test_simulation(self, system_frame):
        ...
        assert value(m.fs.costing.LCOW) == pytest.approx(0.4396, rel=1e-3)
...
"""
    ...
    def test_optimization(self, system_frame):
        ...
        assert value(m.fs.RO.inlet.pressure[0]) == pytest.approx(5.707e6, rel=1e-3)
        assert value(m.fs.RO.area) == pytest.approx(115.0, rel=1e-3)
        # check system metrics
        assert value(m.fs.RO.recovery_vol_phase[0, "Liq"]) == pytest.approx(
            0.4954, rel=1e-3
        )
        ...
        assert value(m.fs.costing.LCOW) == pytest.approx(0.4112, rel=1e-3)
...
class TestROwithTurbine:
    ...
    def test_initialize_system(self, system_frame):
        m = system_frame
        initialize_system(m, solver=solver)
        assert pytest.approx(60.155, rel=1e-5) == value(m.fs.RO.area)
