...
def build():
    ...
    m.fs.S1 = Separator(default={"property_package": m.fs.properties, "outlet_list": ["P1", "PXR"]})
...
def calculate_operating_pressure(
    feed_state_block=None, over_pressure=0.15, water_recovery=0.5, NaCl_passage=0.01, solver=None
):
    """
...
def display_system(m):
    ...
    print(
        "Water recovery: %.1f%%" % (value(m.fs.RO.recovery_mass_phase_comp[0, "Liq", "H2O"]) * 100)
    )
