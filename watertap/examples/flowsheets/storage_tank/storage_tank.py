...
def build():
    ...
    m.fs.suboxicASM = StorageTankZO(property_package=m.fs.prop, database=m.db)

    # connections
    m.fs.s01 = Arc(source=m.fs.feed.outlet, destination=m.fs.suboxicASM.inlet)
    m.fs.s02 = Arc(source=m.fs.suboxicASM.outlet, destination=m.fs.product_H2O.inlet)
...
def display_additional_results(m):
    ...
    print(f"Total capital costs: {total_capital_costs:.4f} $")
    suboxicASM_capital_costs = value(m.fs.suboxicASM.costing.capital_cost)
    print(f"Suboxic ASM capital costs: {suboxicASM_capital_costs:.4f} $")
