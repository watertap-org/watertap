...
def build():
    ...
    m.fs.prop = prop_ZO.WaterParameterBlock(
        solute_list=["cod", "tss", "tkn", "phosphorus"]
    )
    ...
    m.fs.suboxicASM = SludgeTankZO(property_package=m.fs.prop, database=m.db)
...
def display_additional_results(m):
    ...
    print("----------Capital costs----------")
    total_capital_costs = value(m.fs.costing.total_capital_cost)
    print(f"Total capital costs: {total_capital_costs:.0f} $")
    suboxicASM_capital_costs = value(m.fs.suboxicASM.costing.capital_cost)
    print(f"Suboxic ASM capital costs: {suboxicASM_capital_costs:.0f} $")
