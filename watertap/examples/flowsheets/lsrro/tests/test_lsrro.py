...
class _TestLSRRO:
    ...
    def test_build(self, model):
        ...
        for idx in range(1, self.number_of_stages):
            arc_source_sink.append(
                (fs.pump_to_mixer[idx], fs.PrimaryPumps[idx].outlet, fs.Mixers[idx].upstream)
            )
            arc_source_sink.append(
                (fs.mixer_to_stage[idx], fs.Mixers[idx].outlet, fs.ROUnits[idx].inlet)
            )
            arc_source_sink.append(
                (fs.stage_to_pump[idx], fs.ROUnits[idx].retentate, fs.PrimaryPumps[idx + 1].inlet)
            )
        for idx in range(2, self.number_of_stages + 1):
            arc_source_sink.append(
                (fs.stage_to_eq_pump[idx], fs.ROUnits[idx].permeate, fs.BoosterPumps[idx].inlet)
            )
            arc_source_sink.append(
                (
    ...
    def test_set_operating_conditions(self, model):
        ...
        self._test_fixed_value(fs.feed.flow_mass_phase_comp[0, "Liq", "NaCl"], 70.0 / 1000.0)
        self._test_fixed_value(fs.feed.flow_mass_phase_comp[0, "Liq", "H2O"], (1.0 - 70.0 / 1000.0))
