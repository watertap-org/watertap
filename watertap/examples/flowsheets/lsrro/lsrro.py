...
def set_operating_conditions(m):
    ...
    m.fs.feed.flow_mass_phase_comp[0, "Liq", "H2O"].fix(
        feed_flow_mass * (1 - feed_mass_frac_NaCl)
    )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) not in (2, 3):
        ...
        print(
            "Usage 2: python lsrro.py number_of_stages target_water_recovery_fraction"
        )
    elif len(sys.argv) == 2:
        main(int(sys.argv[1]))
    else:
