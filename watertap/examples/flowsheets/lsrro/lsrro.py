...
def build(number_of_stages=2):
    ...
    m.fs.eq_water_recovery = Constraint(
        expr=sum(m.fs.feed.flow_mass_phase_comp[0, "Liq", :]) * m.fs.water_recovery
    ...
    m.fs.mixer_to_stage = Arc(
        m.fs.NonFinal_StageSet,
        rule=lambda fs, n: {"source": fs.Mixers[n].outlet, "destination": fs.ROUnits[n].inlet},
    )
    ...
    m.fs.pump_to_stage = Arc(
        source=m.fs.PrimaryPumps[last_stage].outlet, destination=m.fs.ROUnits[last_stage].inlet
    )

    # Connect Final Stage to EnergyRecoveryDevice Pump
    m.fs.stage_to_erd = Arc(
        source=m.fs.ROUnits[last_stage].retentate, destination=m.fs.EnergyRecoveryDevice.inlet
    )
        ...
        if stage == last_stage:
            ...
            if guess_mixers:
                _lsrro_mixer_guess_initializer(
                    m.fs.Mixers[stage], solvent_multiplier=0.5, solute_multiplier=0.2, optarg=optarg
                )
            else:
...
def display_state(m):
    ...
    def print_state(s, b):
        ...
        print(
            s.ljust(20)
            + ": %.3f kg/s, %.0f ppm, %.1f bar" % (flow_mass, mass_frac_ppm, pressure_bar)
        )
    ...
    for stage in m.fs.StageSet:
        ...
        print(
            f"Stage {stage} Volumetric water recovery: {wr*100:.2f}%, Salt rejection: {sr*100:.2f}%"
        )
