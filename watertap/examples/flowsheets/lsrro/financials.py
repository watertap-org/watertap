...
def add_costing_param_block(self):
    ...
    b.factor_MLC = Var(
        initialize=0.03, doc="Maintenance-labor-chemical factor [fraction of investment cost/year]"
    )
    ...
    b.mem_cost = Var(initialize=30, doc="Membrane cost [$/m2]")
...
def get_system_costing(self):
    ...
    b.capital_cost_total = Var(
        initialize=1e5, domain=NonNegativeReals, doc="Total capital cost [$]"
    )
    b.investment_cost_total = Var(
        initialize=1e5, domain=NonNegativeReals, doc="Total investment cost [$]"
    )
    ...
    b.operating_cost_total = Var(
        initialize=1e4, domain=NonNegativeReals, doc="Total operating cost [$/year]"
    )
    b.LCOW = Var(initialize=1.0, domain=NonNegativeReals, doc="Levelized cost of water [$/m3]")
    ...
    iscale.set_scaling_factor(
        b.eq_capital_cost_total, iscale.get_scaling_factor(b.capital_cost_total)
    )
    ...
    iscale.set_scaling_factor(
        b.eq_investment_cost_total, iscale.get_scaling_factor(b.investment_cost_total)
    )

    b.eq_operating_cost_MLC = Constraint(
        expr=(b.operating_cost_MLC == b.investment_cost_total * self.costing_param.factor_MLC)
    )
    iscale.set_scaling_factor(
        b.eq_operating_cost_MLC, iscale.get_scaling_factor(b.operating_cost_MLC)
    )
    ...
    iscale.set_scaling_factor(
        b.eq_operating_cost_total, iscale.get_scaling_factor(b.operating_cost_total)
    )
...
def _make_vars(self):
    ...
    self.capital_cost = Var(initialize=1e3, domain=NonNegativeReals, doc="Unit capital cost [$]")
    self.operating_cost = Var(
        initialize=1e3, domain=NonNegativeReals, doc="Operating cost [$/year]"
    )
...
def ReverseOsmosis_costing(self):
    ...
    self.eq_operating_cost = Constraint(
        ...
        * b_fs.costing_param.mem_cost
        * b_RO.area
        / pyunits.m ** 2
    )
    iscale.set_scaling_factor(
        self.eq_operating_cost, iscale.get_scaling_factor(self.operating_cost)
    )
...
def pressure_changer_costing(self, pump_type="centrifugal"):
    ...
    if pump_type == "High pressure":
        ...
        iscale.set_scaling_factor(
            self.eq_capital_cost, iscale.get_scaling_factor(self.capital_cost)
        )
        ...
        self.eq_capital_cost = Constraint(
            expr=(
                self.capital_cost
                == b_fs.costing_param.erd_cost["A"]
                * (
                    sum(
                        b_cv_in.flow_mass_phase_comp["Liq", j] / (pyunits.kg / pyunits.s)
                        for j in b_PC.config.property_package.component_list
                    )
                    / (b_cv_in.dens_mass_phase["Liq"] / (pyunits.kg / pyunits.m ** 3))
                    * 3600
                )
                ** 0.58
            )
