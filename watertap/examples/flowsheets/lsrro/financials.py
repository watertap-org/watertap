...
def add_costing_param_block(self):
    ...
    b.lsrro_mem_cost = Var(initialize=50, doc="Membrane cost [$/m2]")
    b.hp_pump_cost = Var(
...
def get_system_costing(self):
    ...
    b.LCOW = Var(
        initialize=1.0, domain=NonNegativeReals, doc="Levelized cost of water [$/m3]"
    )
...
def _make_vars(self):
    ...
    self.capital_cost = Var(
        initialize=1e3, domain=NonNegativeReals, doc="Unit capital cost [$]"
    )
    self.operating_cost = Var(
        initialize=1e3, domain=Reals, doc="Operating cost [$/year]"
    )
...
def pressure_changer_costing(self, pump_type="centrifugal"):
    ...
    if pump_type == "High pressure":
        ...
        self.eq_capital_cost = Constraint(
            ...
            * b_PC.outlet.pressure[0]
            * b_PC.control_volume.properties_out[0].flow_vol
            * pyunits.s
            / (pyunits.m**3 * pyunits.pascal)
        )
        iscale.set_scaling_factor(
            self.eq_capital_cost, iscale.get_scaling_factor(self.capital_cost)
        )
