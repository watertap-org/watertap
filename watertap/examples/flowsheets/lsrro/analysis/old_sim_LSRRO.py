...
def main():
    ...
    for stage in range(4, 5):
        ...
        total_area.append(
            value(sum(m.fs.ROUnits[a].area for a in range(1, m.fs.NumberOfStages + 1)))
        )
        ...
        final_perm.append(
            value(
                ...
                / sum(
                    m.fs.product.flow_mass_phase_comp[0, "Liq", j].value
                    for j in ["H2O", "NaCl"]
                )
            )
        )
        lcow_breakdown[f"primary_pump_capex {stage}"] = value(
            m.fs.costing_param.factor_capital_annualization
            * sum(
                m.fs.PrimaryPumps[stage].costing.capital_cost for stage in m.fs.StageSet
            )
            / m.fs.annual_water_production
        )
        lcow_breakdown[f"booster_pump_capex {stage}"] = value(
            m.fs.costing_param.factor_capital_annualization
            * sum(
                m.fs.BoosterPumps[stage].costing.capital_cost
                for stage in m.fs.LSRRO_StageSet
            )
            / m.fs.annual_water_production
        )
        lcow_breakdown[f"erd_capex {stage}"] = value(
            m.fs.costing_param.factor_capital_annualization
            * m.fs.EnergyRecoveryDevice.costing.capital_cost
            / m.fs.annual_water_production
        )
        lcow_breakdown[f"membrane_capex {stage}"] = value(
            m.fs.costing_param.factor_capital_annualization
            * sum(m.fs.ROUnits[stage].costing.capital_cost for stage in m.fs.StageSet)
            / m.fs.annual_water_production
        )
        lcow_breakdown[f"indirect_capex {stage}"] = value(
            m.fs.costing_param.factor_capital_annualization
            * (m.fs.costing.investment_cost_total - m.fs.costing.capital_cost_total)
            / m.fs.annual_water_production
        )
        lcow_breakdown[f"electricity {stage}"] = value(
            (
                sum(
