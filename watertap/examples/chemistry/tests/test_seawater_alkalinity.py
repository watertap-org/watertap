...
thermo_config = {
    ...
    "inherent_reactions": {
        ...
        # End R3
        "NaHCO3_K1": {
            "stoichiometry": {("Liq", "NaHCO3"): -1, ("Liq", "Na_+"): 1, ("Liq", "HCO3_-"): 1},
            "heat_of_reaction": constant_dh_rxn,
            "equilibrium_constant": van_t_hoff,
            "equilibrium_form": log_power_law_equil,
            "concentration_form": ConcentrationForm.moleFraction,
            "parameter_data": {
                ...
                # By default, reaction orders follow stoichiometry
                #    manually set reaction order here to override
                "reaction_order": {("Liq", "NaHCO3"): -1, ("Liq", "Na_+"): 1, ("Liq", "HCO3_-"): 1},
            }
            # End parameter_data
        }
        # End R4
...
class TestSeawaterAlkalinity:
    ...
    def test_solution_inherent(self, inherent_reactions_config):
        ...
        assert pytest.approx(10.00029, rel=1e-5) == value(model.fs.unit.outlet.flow_mol[0])
        ...
        total_salt = (
            value(model.fs.unit.outlet.mole_frac_comp[0, "Na_+"]) * total_molar_density * 23
        )
        total_salt += (
            value(model.fs.unit.outlet.mole_frac_comp[0, "Cl_-"]) * total_molar_density * 35.44
        )
        total_salt += (
            value(model.fs.unit.outlet.mole_frac_comp[0, "NaHCO3"]) * total_molar_density * 84
        )
        psu = total_salt / (total_molar_density * 18) * 1000
        assert pytest.approx(32.66105, rel=1e-5) == psu

        total_carbonate = (
            value(model.fs.unit.outlet.mole_frac_comp[0, "NaHCO3"]) * total_molar_density
        )
        total_carbonate += (
        ...
        total_carbonate += (
            value(model.fs.unit.outlet.mole_frac_comp[0, "CO3_2-"]) * total_molar_density
        )
    ...
    def test_solution_equilibrium(self, equilibrium_reactions_config):
        ...
        assert pytest.approx(297.9, rel=1e-5) == value(model.fs.unit.outlet.temperature[0])
        assert pytest.approx(10.0002, rel=1e-5) == value(model.fs.unit.outlet.flow_mol[0])
        assert pytest.approx(101325, rel=1e-5) == value(model.fs.unit.outlet.pressure[0])
