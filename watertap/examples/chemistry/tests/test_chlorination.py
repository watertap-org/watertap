...
thermo_config = {
    ...
    "inherent_reactions": {
        ...
        # End R1
        "NH4_Ka": {
            "stoichiometry": {("Liq", "NH4_+"): -1, ("Liq", "H_+"): 1, ("Liq", "NH3"): 1},
            "heat_of_reaction": constant_dh_rxn,
            "equilibrium_constant": van_t_hoff,
            "equilibrium_form": log_power_law_equil,
            "concentration_form": ConcentrationForm.moleFraction,
            "parameter_data": {
                ...
                # By default, reaction orders follow stoichiometry
                #    manually set reaction order here to override
                "reaction_order": {("Liq", "NH4_+"): -1, ("Liq", "H_+"): 1, ("Liq", "NH3"): 1},
            }
            # End parameter_data
        },
        # End R2
...
class TestChlorination:
    ...
    def chlorination_obj(self):
        ...
        model.fs.unit.inlet.mole_frac_comp[0, "OCl_-"].fix(0.0)
        model.fs.unit.inlet.mole_frac_comp[0, "NH4_+"].fix(0.0)

        model.fs.unit.inlet.mole_frac_comp[0, "NH2Cl"].fix(0.0)
        model.fs.unit.inlet.mole_frac_comp[0, "NHCl2"].fix(0.0)
        model.fs.unit.inlet.mole_frac_comp[0, "NCl3"].fix(0.0)
    ...
    def test_solution(self, chlorination_obj):
        ...
        assert pytest.approx(101325.0, rel=1e-5) == value(model.fs.unit.outlet.pressure[0])
        ...
        hypo_remaining = (
            value(
                model.fs.unit.control_volume.properties_out[0.0].conc_mol_phase_comp["Liq", "HOCl"]
            )
            / 1000
        )
        hypo_remaining += (
            value(
                model.fs.unit.control_volume.properties_out[0.0].conc_mol_phase_comp["Liq", "OCl_-"]
            )
            / 1000
        )
        combined_chlorine = 0
        combined_chlorine += (
            value(
                model.fs.unit.control_volume.properties_out[0.0].conc_mol_phase_comp["Liq", "NH2Cl"]
            )
            / 1000
        )
        combined_chlorine += (
            2
            * value(
                model.fs.unit.control_volume.properties_out[0.0].conc_mol_phase_comp["Liq", "NHCl2"]
            )
            / 1000
        )
        combined_chlorine += (
            3
            * value(
                model.fs.unit.control_volume.properties_out[0.0].conc_mol_phase_comp["Liq", "NCl3"]
            )
            / 1000
        )
