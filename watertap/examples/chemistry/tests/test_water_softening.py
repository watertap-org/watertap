...
from pyomo.environ import ConcreteModel, TerminationCondition, SolverStatus, value, Suffix
...
class TestWaterStoich(object):
    ...
    def test_scaling_stoich(self, water_stoich):
        ...
        for i in m.fs.unit.control_volume.properties_out[0.0].mole_frac_phase_comp:
            ...
            iscale.set_scaling_factor(
                m.fs.unit.control_volume.properties_out[0.0].mole_frac_comp[i[1]], 10 / scale
            )
            iscale.set_scaling_factor(
                m.fs.unit.control_volume.properties_out[0.0].mole_frac_phase_comp[i], 10 / scale
            )
            iscale.set_scaling_factor(
                m.fs.unit.control_volume.properties_out[0.0].flow_mol_phase_comp[i], 10 / scale
            )
            iscale.constraint_scaling_transform(
                m.fs.unit.control_volume.properties_out[0.0].component_flow_balances[i[1]],
                10 / scale,
            )
            iscale.constraint_scaling_transform(
    ...
    def test_solution_inherent(self, water_stoich):
        ...
        total_hardness1 = (
            50000 * 2 * m.fs.unit.outlet.mole_frac_comp[0, "Ca(HCO3)2"].value * total_molar_density
        )
        total_hardness2 = (
            50000 * 2 * m.fs.unit.outlet.mole_frac_comp[0, "Mg(HCO3)2"].value * total_molar_density
        )
