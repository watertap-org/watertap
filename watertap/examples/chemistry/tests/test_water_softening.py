...
class TestWaterStoich(object):
    ...
    def test_solution(self, water_stoich):
        m = water_stoich
        total_molar_density = (
            value(m.fs.unit.control_volume.properties_out[0.0].dens_mol_phase["Liq"])
            / 1000
        )
        total_hardness1 = (
            50000
            * 2
            * m.fs.unit.outlet.mole_frac_comp[0, "Ca(HCO3)2"].value
            * total_molar_density
        )
        total_hardness2 = (
            50000
            * 2
            * m.fs.unit.outlet.mole_frac_comp[0, "Mg(HCO3)2"].value
            * total_molar_density
        )
