...
class TestENRTLwater:
    ...
    def test_solution_water(self, water_model):
        ...
        pH = -log10(
            value(model.fs.unit.control_volume.properties_out[0.0].act_phase_comp["Liq", "H_+"])
            * 55.2
        )
        pOH = -log10(
            value(model.fs.unit.control_volume.properties_out[0.0].act_phase_comp["Liq", "OH_-"])
            * 55.2
        )
...
class TestENRTLcarbonicAcid:
    ...
    def test_solution_carbonic_acid(self, carbonic_acid_model):
        ...
        total_acid = (
            value(
                model.fs.unit.control_volume.properties_out[0.0].conc_mol_phase_comp["Liq", "H2CO3"]
            )
            / 1000
        )
        total_acid += (
