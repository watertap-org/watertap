...
The following parameters are constructed for the unit on the FlowsheetCostingBlock (e.g., `m.fs.costing.gac`) when applying the `cost_gac` costing method in the ``watertap_costing_package``:
